...
cdef class CyKNGraph:
    ...
    def sigmoid(self, DTensor input):
        cdef CppDTensor* ptr = self.p_kgraph.sigmoid(input.c_ptr)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return DTensor(t)

    def log(self, DTensor input):
        cdef CppDTensor* ptr = self.p_kgraph.log(input.c_ptr)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return DTensor(t)
    ...
    def concat(self, DTensor A, DTensor B, int dim):
        cdef CppDTensor* ptr = self.p_kgraph.concat(A.c_ptr, B.c_ptr, dim)
        if ptr == NULL:
            raise ValueError("concat failed: shape mismatch or allocation error")
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return DTensor(t)

    def split(self, DTensor input, int split_size, int dim):
        cdef int n = self.p_kgraph.split(input.c_ptr, split_size, dim)
        if n != 2:
            raise ValueError("split failed: invalid split_size, dim, or shape")
        cdef CppKNOperator* op = self.p_kgraph.operators.back()
        cdef CppDTensor* coutputs[2]
        cdef int got = op.get_output_dtensors(coutputs)
        if got != 2:
            raise RuntimeError(f"split output count mismatch: expected 2, got {got}")
        outputs = []
        cdef object t
        for i in range(2):
    ...
    def chunk(self, DTensor input, int chunk_size, int dim):
        cdef int n = self.p_kgraph.chunk(input.c_ptr, chunk_size, dim)
        if n <= 0:
            raise ValueError("chunk failed: invalid chunk_size, dim, or shape")
        cdef CppKNOperator* op = self.p_kgraph.operators.back()
        cdef CppDTensor* coutputs[64]
        if n > 64:
            raise ValueError(f"chunk produced {n} outputs; max 64 supported in bindings")
        cdef int got = op.get_output_dtensors(coutputs)
        if got != n:
            raise RuntimeError(f"chunk output count mismatch: expected {n}, got {got}")
        outputs = []
        cdef object t
        for i in range(n):
            t = ctypes.cast(<unsigned long long>coutputs[i], ctypes.c_void_p)
    ...
    def _get_tb_operator_info(self, CyTBOperator op):
        ...
        elif op.op_type == "tb_mul_scalar_op":
            extra = pyjson.loads(tb_operator_json_dump(op.c_ptr).decode("utf-8"))
            if "scalar" in extra:
                ans["scalar"] = extra["scalar"]
            elif tb_operator_mul_scalar_value(op.c_ptr, &scalar):
                ans["scalar"] = scalar
        elif op.op_type == "tb_clamp_op":
            extra = pyjson.loads(tb_operator_json_dump(op.c_ptr).decode("utf-8"))
            for key in ("min_val", "max_val"):
                if key in extra:
                    ans[key] = extra[key]
            if "min_val" not in ans or "max_val" not in ans:
                if tb_operator_clamp_bounds(op.c_ptr, &min_val, &max_val):
                    ans["min_val"] = min_val
                    ans["max_val"] = max_val
        return ans
    ...
    def _get_kn_operator_info(self, CyKNOperator op):
        ...
        if op.op_type == "kn_customized_op":
            ...
            if op.op_type == "kn_clamp_op":
                if kn_operator_clamp_bounds(op.c_ptr, &min_val, &max_val):
                    ans["min_val"] = min_val
                    ans["max_val"] = max_val
...
cdef class CyTBGraph:
    ...
    def sigmoid(self, STensor A):
        cdef CppSTensor* ptr = self.p_bgraph.sigmoid(A.c_ptr)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return STensor(t)

    def log(self, STensor A):
        cdef CppSTensor* ptr = self.p_bgraph.log(A.c_ptr)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return STensor(t)
    ...
    def pow(self, STensor A, STensor B):
        cdef CppSTensor* ptr = self.p_bgraph.pow(A.c_ptr, B.c_ptr)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return STensor(t)
    ...
    def reduction_to_dimx(self, STensor A, int dim):
        cdef CppSTensor* ptr = self.p_bgraph.reduction_to_dimx(A.c_ptr, dim)
        t = ctypes.cast(<unsigned long long>ptr, ctypes.c_void_p)
        return STensor(t)
    ...
    def split(self, STensor A, int split_size, int dim):
        cdef vector[CppSTensor*] ptr = self.p_bgraph.split(A.c_ptr, split_size, dim)
        if ptr.size() != 2:
            raise ValueError("split failed: invalid split_size, dim, or shape")
        t0 = ctypes.cast(<unsigned long long>ptr[0], ctypes.c_void_p)
        t1 = ctypes.cast(<unsigned long long>ptr[1], ctypes.c_void_p)
    ...
    def chunk(self, STensor A, int chunk_size, int dim):
        cdef vector[CppSTensor*] ptr = self.p_bgraph.chunk(A.c_ptr, chunk_size, dim)
        if ptr.size() <= 0:
            raise ValueError("chunk failed: invalid chunk_size, dim, or shape")
        outputs = []
        cdef object t
        ...
        for i in range(ptr.size()):
            t = ctypes.cast(<unsigned long long>ptr[i], ctypes.c_void_p)
            outputs.append(STensor(t))
        return outputs
...
def search(CyKNGraph input_graph, *, str backend = "cuda", int max_num_new_graphs = 8192, list imaps = None, list omaps = None, list griddims = None, list blockdims = None, list fmaps = None, list franges = None, str previous_checkpoint = None, bool verbose, str default_config = None, bool is_formal_verified):
    ...
    # set blockdims (now fully supported)
...
def get_available_backends():
    """Get list of available backends from C++"""
    # Ensure backends are initialized first
    initialize_backends()
    cdef vector[string] backends = get_available_backend_names()
    return [b.decode('utf-8') for b in backends]

def check_backend_available(backend_name: str):
    """Check if a backend is available"""
    cdef string name_str = backend_name.encode('utf-8')
    return is_backend_available(name_str)
...
cdef extern from "kernel/cpu/cpu_kernel_c.h":
    int yirage_cpu_gemm_f32(const float *A, const float *B, float *C, int M,
                            int N, int K, int num_threads) nogil
    int yirage_cpu_rms_matmul_f32(const float *X, const float *W, float *Y,
                                  int M, int N, int K, float epsilon,
                                  int num_threads) nogil


def cpu_gemm_f32(a, b, c, int m, int n, int k, int num_threads=1):
    """Native CPU GEMM: C = A @ B using detected SIMD (AVX2/AVX-512/NEON)."""
    cdef unsigned long long pa_val = ctypes.cast(a.data_ptr(), ctypes.c_void_p).value
    cdef unsigned long long pb_val = ctypes.cast(b.data_ptr(), ctypes.c_void_p).value
    cdef unsigned long long pc_val = ctypes.cast(c.data_ptr(), ctypes.c_void_p).value
    cdef float* pa = <float*>pa_val
    cdef float* pb = <float*>pb_val
    cdef float* pc = <float*>pc_val
    cdef int rc
    with nogil:
        rc = yirage_cpu_gemm_f32(pa, pb, pc, m, n, k, num_threads)
    return rc


def cpu_rms_matmul_f32(x, w, y, int m, int n, int k, float epsilon=1e-6,
                       int num_threads=0):
    """Native fused RMS+GEMM (OpenMP + cblas when built with BLAS)."""
    cdef unsigned long long px_val = ctypes.cast(x.data_ptr(), ctypes.c_void_p).value
    cdef unsigned long long pw_val = ctypes.cast(w.data_ptr(), ctypes.c_void_p).value
    cdef unsigned long long py_val = ctypes.cast(y.data_ptr(), ctypes.c_void_p).value
    cdef float* px = <float*>px_val
    cdef float* pw = <float*>pw_val
    cdef float* py = <float*>py_val
    cdef int rc
    with nogil:
        rc = yirage_cpu_rms_matmul_f32(px, pw, py, m, n, k, epsilon, num_threads)
    return rc
