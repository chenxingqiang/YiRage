    ...
    def compile(self, config: CompileConfig) -> CompileResult:
        ...
        if not self.is_available():
            return CompileResult(
                success=False, output_path="", error_message=f"{self.name} compiler not available"
            )
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=source_ext, delete=False, dir=config.output_dir
        ) as f:
        ...
        try:
            ...
            return CompileResult(
                success=False, output_path="", error_message="Compilation timed out"
            )
        except Exception as e:
            return CompileResult(success=False, output_path="", error_message=str(e))

    def _get_output_name(self, config: CompileConfig) -> str:
        """Generate output file name."""
        return (
            f"kernel.cpython-{sys.version_info.major}{sys.version_info.minor}-x86_64-linux-gnu.so"
        )
...
class MXCCCompiler(BaseCompiler):
    ...
    def is_available(self) -> bool:
        if self._compiler_path is None:
            if self._maca_path:
                self._compiler_path = shutil.which("mxcc") or os.path.join(
                    self._maca_path, "mxgpu_llvm/bin/mxcc"
                )
    ...
    def is_available(self) -> bool:
        if self._compiler_path is None:
            ...
            if not self._compiler_path and self._ascend_path:
                self._compiler_path = os.path.join(self._ascend_path, "compiler/bin/ascendc")
...
class TritonCompiler(BaseCompiler):
    ...
    def compile(self, config: CompileConfig) -> CompileResult:
        ...
        if not self.is_available():
            return CompileResult(
                success=False, output_path="", error_message="Triton not available"
            )
        ...
        return CompileResult(success=True, output_path=output_path, metadata={"jit": True})
...
def compile_kernel(
    source_code: str, output_dir: str, backend: str = "auto", target_arch: str = "native", **kwargs
) -> CompileResult:
    """
    ...
    if compiler is None:
        return CompileResult(
            success=False, output_path="", error_message=f"No compiler available for {backend}"
        )
