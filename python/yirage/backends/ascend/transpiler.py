...
def generate_triton_kernel_for_ascend(
    kernel_spec: Dict[str, Any], config: AscendTranspileConfig
) -> str:
    """
...
def transpile_to_ascend(
    kernel_spec: Dict[str, Any], config: Optional[AscendTranspileConfig] = None
    ...
    if use_cpp_transpiler() and _cpp_ascend_transpile is not None:
        try:
            ...
            return AscendTranspileResult(
                ...
                error_message=(
                    cpp_result.error_message if hasattr(cpp_result, "error_message") else ""
                ),
            )
...
def compile_ascend_kernel(
    ...
    try:
        subprocess.check_call(result.compile_command.split(), cwd=output_dir, timeout=300)
