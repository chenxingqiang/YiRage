...
def get_available_backends() -> List[str]:
    ...
    """
    if not HAS_CORE:
        return _detect_python_backends()

    try:
        if hasattr(core, "get_available_backends"):
            result = core.get_available_backends()
            if result:
                return result
...
def _detect_python_backends() -> List[str]:
    """Detect backends using Python-level checks (fallback when C++ is unavailable or empty)."""
    backends = ["cpu"]
    try:
        import torch
