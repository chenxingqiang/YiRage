...
from ..core import *
from ..kernel.graph import get_key_paths, KNGraph
...
def get_compile_command(
    ...
    common_cmd = [
        ...
        f"-I{py_include_dir}",
        f"-I{yirage_inc_path}",
        f"-I{os.path.join(yirage_inc_path, 'persistent_kernel')}",
        f"-I{os.path.join(yirage_deps_path, 'cutlass/include')}",
        f"-I{os.path.join(yirage_deps_path, 'cutlass/tools/util/include')}",
    ...
    flags = flags + [f"-DYPK_TARGET_CC={target_cc}", "-DYIRAGE_BACKEND_USE_CUDA"]
    ...
    flags = flags + [f"-DYPK_MAX_NUM_BATCHED_REQUESTS={ypk.max_num_batched_requests}"]
    flags = flags + [f"-DYPK_MAX_NUM_BATCHED_TOKENS={ypk.max_num_batched_tokens}"]
    flags = flags + [f"-DYPK_MAX_NUM_PAGES={ypk.max_num_pages}"]
    flags = flags + [f"-DYPK_PAGE_SIZE={ypk.page_size}"]
    flags = flags + [f"-DYPK_MAX_SEQ_LENGTH={ypk.max_seq_length}"]
    # Use when debugging
    # flags = flags + [f"-DYPK_ENABLE_VERBOSE"]
    ...
    if profiling:
        flags = flags + ["-DYPK_ENABLE_PROFILING"]
...
class PersistentKernel:
    ...
    def attach_input(self, torch_tensor: torch.Tensor, name: str = None) -> DTensor:
        ...
        # Name is required for tensor binding in persistent kernel runtime
        if name is None:
            raise ValueError("name is required for attach_torch_tensor")
    ...
    def new_tensor(
        ...
        # Name is required for tensor binding in persistent kernel runtime
        if name is None:
            raise ValueError("name is required for new_tensor")
    ...
    def embed_layer(
        ...
        # Note: Current implementation is optimized for batch_size=1 (speculative decoding)
        # For batch_size > 1, use the general embedding implementation via KNGraph
        ...
        self.kn_graph.register_task(
            tb_graph, "embedding" if self.target_cc == 90 else "embedding", [input_source]
        )
    ...
    def identity_layer(
        ...
        # Identity layer copies input to output (for residual connections)
        # Used in persistent kernel for explicit data movement control
        last_dim = 0
    ...
    def argmax_reduce_layer(
        ...
        # Output maps batch dim to grid.x, task dim to grid.y
        tb_graph.new_input(output, (0, 1, -1), -1, True)
    ...
    def prompt_lookup_verify_handler(
        ...
        self.target_verify_greedy_layer(
            input=(spec_tokens, target_output),
            output=verify_out,
            grid_dim=grid_dim,
            block_dim=block_dim,
        )
    ...
    def __call__(self, **kwargs):
        ...
        if self.profiler_tensor is not None:
            ...
            if self.trace_name:
                ...
                trace_name = f"yirage_{self.mpi_rank}.perfetto-trace"
