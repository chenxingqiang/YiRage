...
from ..backends.api import get_available_backends, get_default_backend
...
class CUDAKernel(BackendKernel):
    ...
    def compile(self, source_code: str, config: KernelConfig) -> CompileResult:
        ...
        compile_config = CompileConfig(
            ...
            include_dirs=[
                os.path.join(INCLUDE_PATH, "transpiler/runtime"),
        ...
        if result.success:
            ...
            spec = importlib.util.spec_from_file_location("__yirage_launcher", result.output_path)
    ...
    def execute(self, ctx: KernelExecutionContext) -> None:
        ...
        if self._buffer is None:
            ...
            self._buffer = torch.empty(
                1024 * 1024, dtype=torch.uint8, device=ctx.input_tensors[0].device  # 1MB buffer
            )
        ...
        # Execute
        self._launcher(input_ptrs, output_ptrs, buffer_ptr, stream.cuda_stream, profiler_ptr)
...
class MACAKernel(BackendKernel):
    ...
    def execute(self, ctx: KernelExecutionContext) -> None:
        ...
        if self._buffer is None:
            self._buffer = torch.empty(
                1024 * 1024, dtype=torch.uint8, device=ctx.input_tensors[0].device
            )
        ...
        self._launcher(input_ptrs, output_ptrs, self._buffer.data_ptr(), stream.cuda_stream, 0)
    ...
    def compile(self, source_code: str, config: KernelConfig) -> CompileResult:
        ...
        if transpile_result.success:
            ...
            return CompileResult(
                success=True, output_path=kernel_path, metadata={"triton_jit": True}
            )

        return CompileResult(
            success=False, output_path="", error_message=transpile_result.error_message
        )
...
class CPUKernel(BackendKernel):
    ...
    def compile(self, source_code: str, config: KernelConfig) -> CompileResult:
        ...
        if compiler is None:
            return CompileResult(
                success=False, output_path="", error_message="No CPU compiler available"
            )
...
class MultiBackendKernel:
    ...
    def _auto_detect_config(self) -> KernelConfig:
        ...
        config.fallback_backends = [b for b in fallback_priority if b != config.backend]
...
def create_kernel(source_code: str, backend: str = "auto", **kwargs) -> MultiBackendKernel:
    """
