...
""",
    ...
    for next_fn in op_node.fn.next_functions:
        if id(next_fn[0]) in ids_to_nodes:
            ...
            if next_fn[0] == None:
                continue
            ids_to_nodes[id(next_fn[0])] = Operator(
                name=next_fn[0].name(), fn=next_fn[0], input_ops=[], output_ops=[op_node]
            )
...
def get_partitions(op_node, max_num_ops, visited_start_nodes, all_subgraphs, UNSUPPORTED_OPS):
    ...
    if op_node.name not in UNSUPPORTED_OPS:
        get_partitions_helper(
            op_node, {op_node: None}, max_num_ops, set(), all_subgraphs, UNSUPPORTED_OPS
        )

    for input_node in op_node.input_ops:
        if id(input_node.fn) not in visited_start_nodes:
            get_partitions(
                input_node, max_num_ops, visited_start_nodes, all_subgraphs, UNSUPPORTED_OPS
            )


def get_partitions_helper(
    op_node, curr_subgraph, max_num_ops, visited, all_subgraphs, UNSUPPORTED_OPS
):
...
def partition_graph(
    ...
    max_num_ops=3,
    UNSUPPORTED_OPS=set(
        ["torch::autograd::AccumulateGrad", "NllLossBackward0", "EmbeddingBackward0"]
    ),
):
