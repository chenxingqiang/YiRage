...
svg_colors = [
    color for color in mcolors.CSS4_COLORS.keys() if color not in dark and "dark" not in color
]
...
def get_color_map(prefix: Literal["TASK", "EVENT"]) -> dict[int, tuple[str, str]]:
    ...
    filename = os.path.join(
        os.environ["YIRAGE_HOME"], "include/persistent_kernel/runtime_header.h"
    )
...
def check_supported_data_types() -> None:
    filename = os.path.join(os.environ["YIRAGE_HOME"], "include/type.h")
    with open(filename, "r") as f:
        lines = f.readlines()
    lines_set = set([line.strip() for line in lines])
    for number, (name, _) in supported_data_types.items():
        assert (
            f"DT_{name.upper()} = {number}," in lines_set
        ), f"DT_{name.upper()} = {number} is not found in {filename}"
    ...
    with open(task_graph_json_filename, "r") as file:
        ...
        for task_idx, task in enumerate(task_graph["all_tasks"]):
            ...
            if task["inputs"] is not None:
                input_str = "\n".join(
                    [
                        f"in: {input['base_ptr']} + {get_offset_in_number_of_elements(input)}"
            ...
            if task["outputs"] is not None:
                output_str = "\n".join(
                    [
                        f"out: {output['base_ptr']} + {get_offset_in_number_of_elements(output)}"
