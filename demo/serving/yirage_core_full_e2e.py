...
"""YiRage core full-layer hybrid MLP e2e (real execution).

Every decoder layer routes MLP through RuntimeFusion ``yirage_cpu`` capsules.
Requires built ``yirage.core`` (see ``scripts/setup_serving_yirage_core.sh``)::
    ...
    python3 demo/serving/yirage_core_full_e2e.py --quick
"""
...
def main() -> int:
    ...
    report = serving.run_yirage_core_full_layer_e2e(
    ...
    payload["yirage_core"] = True

    if args.json:
        print(json.dumps(payload, indent=2))
    else:
        print("YiRage core full-layer hybrid MLP e2e")
