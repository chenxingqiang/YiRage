...
def get_rms_linear():
    ...
    W = graph.new_input(
        dims=(4096, n_local_heads * head_dim + 2 * n_local_kv_heads * head_dim), dtype=yr.float16
    )
    tb_graph = yr.new_threadblock_graph(
        grid_dim=(384, 1, 1), block_dim=(128, 1, 1), forloop_range=32, reduction_dimx=64
    )
...
def get_rms_linear2():
    ...
    tb_graph = yr.new_threadblock_graph(
        grid_dim=(344, 1, 1), block_dim=(128, 1, 1), forloop_range=32, reduction_dimx=64
    )
    tX = tb_graph.new_input(dtensor=X, input_map=(-1, -1, -1), forloop_dim=1)
    tW = tb_graph.new_input(dtensor=W, input_map=(1, -1, -1), forloop_dim=0)
    tM = tb_graph.matmul(tX, tW)
...
def get_chameleon_attention():
    ...
    tbgraph1 = yr.new_threadblock_graph(
        grid_dim=(n_local_kv_heads, 16, 1),
        block_dim=(128, 1, 1),
        forloop_range=4,
        reduction_dimx=128,
    )
...
if __name__ == "__main__":
    X = torch.randn(num_tokens, 4096, dtype=torch.float16, device=_DEVICE)
    Wqkv = torch.randn(
        4096,
    ...
    W13 = torch.randn(4096, intermediate_size * 2, dtype=torch.float16, device=_DEVICE)
    W2 = torch.rand(intermediate_size, 4096, dtype=torch.float16, device=_DEVICE)
    Kcache = torch.rand(
    ...
    mean_syn = __bench_ms(lambda: yirage_chameleon(X, Wqkv, Wo, W13, W2, Kcache, Vcache, kernels))
    print(mean_syn)
