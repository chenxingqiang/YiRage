...
rms_norm = torch.nn.RMSNorm(64, device=_DEVICE, dtype=torch.float16)
...
if __name__ == "__main__":
    ...
    tbgraph1 = yr.new_threadblock_graph(
        grid_dim=(8, 16, 4), block_dim=(128, 1, 1), forloop_range=4, reduction_dimx=64
    )
    ...
    tbgraph2 = yr.new_threadblock_graph(
        grid_dim=(8, 16, 1), block_dim=(128, 1, 1), forloop_range=1, reduction_dimx=64
    )
    ...
    input_tensors = [
        torch.randn(8, 128, 64, dtype=torch.float16, device=_DEVICE),
        torch.randn(8, 64, 4096, dtype=torch.float16, device=_DEVICE),
        torch.randn(8, 4096, 64, dtype=torch.float16, device=_DEVICE),
    ]
