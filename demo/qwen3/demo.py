...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-num-batched-tokens", default=8, type=int, help="Max number of tokens in a batch"
    )
    parser.add_argument(
    ...
    prompt_lengths = torch.full(
        (total_num_requests,), model_inputs.input_ids.shape[-1], dtype=torch.int, device=_DEVICE
    )
    ...
    if args.use_yirage:
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            ypk.linear_layer(
                ...
                output=attn_in,
                grid_dim=(
                    grid_for_rmsnorm_linear_layer(w_qkv.dim(0), args.use_cutlass_kernel),
                    1,
                    1,
                ),
                block_dim=(128, 1, 1),
            ...
            rmsnorm_num_tasks = grid_for_rmsnorm_linear_layer(
                w_gate_proj.dim(0) + w_up_proj.dim(0), args.use_cutlass_kernel
            )
