...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-num-batched-tokens", default=1, type=int, help="Max number of tokens in a batch"
    )
    parser.add_argument(
        "--max-num-batched-requests", default=1, type=int, help="Max number of requests in a batch"
    )
    ...
    parser.add_argument(
        "--ignore-eos", action="store_true", help="Ignore eos token during generation"
    )
    ...
    if _DEVICE != "cuda":
        print("This demo requires CUDA. Exiting.")
        exit(0)
    ...
    total_num_requests = 1 if not args.use_yirage else args.max_num_batched_requests
    # get all model weight tensors
    tokens = torch.full(
        (total_num_requests, args.max_seq_length), 0, dtype=torch.long, device=_DEVICE
    )
    ...
    # get all model weight tensors
    input_tokens = torch.full((args.max_num_batched_tokens, 1), 0, dtype=torch.long, device=_DEVICE)
    output_tokens = torch.full((args.max_num_batched_tokens, 1), 0, dtype=torch.long, device=_DEVICE)
    prev_pos = 0

    starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    step = torch.full((total_num_requests,), 0, dtype=torch.int32, device=_DEVICE)
    num_new_tokens = torch.full((total_num_requests,), 1, dtype=torch.int32, device=_DEVICE)
    ...
    if args.use_yirage:
        ...
        lm_head_weight = torch.cat(
            (
                ...
                torch.full((153600 - model.config.vocab_size, hidden_size), 0, device=_DEVICE),
            ),
            0,
        )
        ...
        if args.profiling:
            profiler_tensor = torch.zeros(
                3000 * 128, dtype=torch.uint64, device=_DEVICE
            ).contiguous()
        else:
        ...
        qo_indptr_buffer = torch.empty(
            args.max_num_batched_requests + 1, dtype=torch.int32, device=_DEVICE
        )
        paged_kv_indptr_buffer = torch.empty(
            args.max_num_batched_requests + 1, dtype=torch.int32, device=_DEVICE
        )
        paged_kv_indices_buffer = torch.empty(args.max_num_pages, dtype=torch.int32, device=_DEVICE)
        paged_kv_last_page_len_buffer = torch.empty(
            args.max_num_batched_requests, dtype=torch.int32, device=_DEVICE
        )
        ...
        if spec_decode_config and spec_decode_config.method == "promptlookup":
            all_tokens = ypk.attach_input(torch_tensor=tokens, name="all_tokens")
        ...
        rmsnorm_out_qkv = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
        ...
        rmsnorm_out_moe = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
        ...
        allreduce_buf = ypk.new_tensor(
            dims=(world_size, args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
            name="all_reduce_buf",
            io_category="nvshmem_tensor" if world_size > 1 else "cuda_tensor",
        )
        attn_allreduce_out = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
            name="attn_allreduce_out",
            io_category="nvshmem_tensor" if world_size > 1 else "cuda_tensor",
        )
        moe_gate_out = ypk.new_tensor(
        ...
        for layer in model.model.layers:
            ...
            for expert_id in range(num_experts):
                ...
                moe_down_proj_torch.append(expert.down_proj.weight.detach().clone())
            moe_gate_up_proj_torch_weights.append(torch.stack(moe_gate_up_proj_torch, dim=0))
            moe_down_proj_torch_weights.append(torch.stack(moe_down_proj_torch, dim=0))
            _sync()
            del layer.mlp.experts

        moe_routing_indices = ypk.new_tensor(
            dims=(num_experts, args.max_num_batched_tokens),
            dtype=yr.int32,
            name="moe_routing_indices",
            io_category="cuda_tensor",
        )
        moe_mask = ypk.new_tensor(
            dims=(num_experts + 1,),
            dtype=yr.int32,
            name="moe_mask",
            io_category="cuda_tensor",
        )
        moe_topk_weight = ypk.new_tensor(
        ...
        mlp_weighted_sum_out = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
            name="mlp_weighted_sum_out",
            io_category="nvshmem_tensor" if world_size > 1 else "cuda_tensor",
        )
        mlp_final = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, hidden_size),
            dtype=yr.bfloat16,
            name="mlp_final",
            io_category="nvshmem_tensor" if world_size > 1 else "cuda_tensor",
        )
        argmax_in = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, vocab_size),
            dtype=yr.bfloat16,
            name="argmax_in",
            io_category="cuda_tensor",
        )
        argmax_part_value = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, ypk.num_workers),
            dtype=yr.bfloat16,
            name="argmax_part_value",
            io_category="cuda_tensor",
        )
        argmax_part_index = ypk.new_tensor(
            dims=(args.max_num_batched_tokens, ypk.num_workers),
            dtype=yr.int64,
            name="argmax_part_index",
            io_category="cuda_tensor",
        )
        argmax_out = ypk.attach_input(torch_tensor=output_tokens, name="output_token")
        ...
        if spec_decode_config:
            ...
            x = spec_tokens
        # Add Embed
        w = ypk.attach_input(torch_tensor=model.model.embed_tokens.weight, name="embed_tokens")
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            ypk.rmsnorm_layer(
                ...
                grid_dim=(ypk.max_num_batched_tokens, 1, 1),
                block_dim=(256, 1, 1),
            )
            ypk.linear_layer(
            ...
            k_cache = ypk.attach_input(torch_tensor=key_cache_torch[i], name=f"layer_{i}_k_cache")
            v_cache = ypk.attach_input(torch_tensor=value_cache_torch[i], name=f"layer_{i}_v_cache")
            ...
            if spec_decode_config:
                ...
                ypk.paged_attention_layer(
                    ...
                    sin_pos_embed=sin_pos_embed,
                    output=attn_out,
                    grid_dim=(ypk.max_num_batched_requests, num_local_kv_heads, 1),
                    block_dim=(256, 1, 1),
                )
            ...
            # topk+softmax
            ypk.moe_topk_softmax_routing_layer(
                input=moe_gate_out,
            ...
            # silu_mul
            ypk.moe_silu_mul_layer(
                input=mlp_mid,
                output=silu_mul_out,
                grid_dim=(ypk.max_num_batched_tokens, num_experts_per_tok, 1),
                block_dim=(256, 1, 1),
            )
            # moe w2 linear
            ypk.moe_w2_linear_layer(
                input=silu_mul_out,
                weight=w_down_proj,
                moe_routing_indices=moe_routing_indices,
                moe_mask=moe_mask,
            ...
            # moe mul sum add
            ypk.moe_mul_sum_add_layer(
                input=mlp_out,
                weight=moe_topk_weight,
                residual=x,
                output=mlp_weighted_sum_out,
                grid_dim=(ypk.max_num_batched_tokens, hidden_size // 256, 1),
                block_dim=(256, 1, 1),
            )
            # reset residual input as x
        ...
        # add rmsnorm_linear layer
        w_norm = ypk.attach_input(torch_tensor=model.model.norm.weight, name="model_norm_weight")
        w_proj = ypk.attach_input(torch_tensor=lm_head_weight, name="lm_head")
        ypk.rmsnorm_layer(
            input=x,
            weight=w_norm,
            output=rmsnorm_out,
        ...
        if spec_decode_config and spec_decode_config.method == "promptlookup":
            argmax_partial_grid_dim = (
                max_factor_leq_n(153600, 96 // (spec_decode_config.spec_length + 1)),
    ...
    if not args.use_yirage:
        ...
        print(
            "Prompt length {}, generate length {}, per-token latency (both prefill and decode): {:.3f} ms".format(
                prompt_lengths[0],
                step.max().item() + 1 - prompt_lengths[0],
