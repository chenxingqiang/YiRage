...
def build_yirage_graph(model, world_size, rank, args, tokens_tensor, step_tensor):
    ...
    # --- Profiler Setup ---
    profiler_tensor = (
        torch.empty(3000 * 128, dtype=torch.uint64, device=_DEVICE).contiguous()
        if args.profiling
        else None
    )
    ...
    # --- Tensor Definitions ---
    input_tokens = torch.full((1, 1), 0, dtype=torch.long, device=_DEVICE)
    ...
    attn_in = ypk.new_tensor(
        dims=(batch_size, fused_outdim_1 // world_size), dtype=yr.bfloat16, name="attn_in"
    )
    attn_out = ypk.new_tensor(
        dims=(batch_size, num_local_q_heads * head_dim), dtype=yr.bfloat16, name="attn_out"
    )
    ...
    mlp_mid = ypk.new_tensor(
        dims=(batch_size, fused_outdim_2 // world_size), dtype=yr.bfloat16, name="mlp_mid"
    )
    mlp_out = ypk.new_tensor(
        dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="mlp_out", io_category=is_nvshmem
    )
    mlp_final = ypk.new_tensor(
        dims=(batch_size, hidden_size), dtype=yr.bfloat16, name="mlp_final", io_category=is_nvshmem
    )
    argmax_in = ypk.new_tensor(dims=(batch_size, vocab_size), dtype=yr.bfloat16, name="argmax_in")
    argmax_part_value = ypk.new_tensor(
        dims=(batch_size, 96), dtype=yr.bfloat16, name="argmax_part_value"
    )
    argmax_part_index = ypk.new_tensor(
        dims=(batch_size, 96), dtype=yr.int64, name="argmax_part_index"
    )
    ...
    ypk.embed_layer(
        input=x, weight=w_embed, output=embed_out, grid_dim=(1, 1, 1), block_dim=(128, 1, 1)
    )
    x = embed_out

    for i, layer in enumerate(model.model.layers):
        # Attention block
        w_norm_attn = ypk.attach_input(
            torch_tensor=layer.input_layernorm.weight, name=f"layer_{i}_input_layernorm"
        )
        ...
        w_qkv = ypk.fuse_tensors(
            inputs=[w_q, w_k, w_v],
            fused_dim=0,
            num_groups=num_local_kv_heads,
            name=f"layer_{i}_qkv_proj",
        )
        ypk.rmsnorm_linear_layer(
            input=x,
            weight_norm=w_norm_attn,
            weight_linear=w_qkv,
            output=attn_in,
            grid_dim=(96, 1, 1),
            block_dim=(128, 1, 1),
        ...
        ypk.silu_mul_linear_with_residual_layer(
            input=mlp_mid,
            weight=w_down_proj,
    ...
    ypk.rmsnorm_linear_layer(
        input=x,
        weight_norm=w_final_norm,
        weight_linear=w_lm_head,
        output=argmax_in,
        grid_dim=(96, 1, 1),
        block_dim=(128, 1, 1),
    )
...
def run_yirage_generation(model, ypk, tokens, prompt_len, step_tensor, position_embeddings):
    ...
    profiler_buffer_ptr = ypk.profiler_tensor.data_ptr() if ypk.profiler_tensor is not None else 0
    ypk.init_func(
        meta_tensors_ptr,
        profiler_buffer_ptr,
        ypk.mpi_rank,
...
def main():
    ...
    parser.add_argument(
        "--use-yirage", action="store_true", help="Use YiRage kernels for inference."
    )
    parser.add_argument(
        "--profiling", action="store_true", help="Enable profiler to generate a trace."
    )
    ...
    tokens = torch.full((1, MAX_SEQ_LEN), 0, dtype=torch.long, device=_DEVICE)
    step_tensor = torch.tensor([0], dtype=torch.int32, device=_DEVICE)
