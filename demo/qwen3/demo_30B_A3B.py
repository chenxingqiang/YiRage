    ...
    if args.use_yirage:
        ...
        for layer in model.model.layers:
            ...
            for expert_id in range(num_experts):
                moe_gate_up_proj_torch.append(
                    torch.concat(
                        [
                            layer.mlp.experts[expert_id].gate_proj.weight,
                            layer.mlp.experts[expert_id].up_proj.weight,
                        ],
                        dim=0,
                    )
                )
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            ypk.linear_layer(
                ...
                output=attn_in,
                grid_dim=(
                    grid_for_rmsnorm_linear_layer(w_qkv.dim(0)),
                    1,
                    1,
                ),  # 5120 split into 64 tasks
                block_dim=(256, 1, 1),
            )
            # add attention
    ...
    if not args.use_yirage:
        ...
        with torch.inference_mode():
            ...
            print(
                "decode step 1 out",
                out.logits[:, -1, :5],
                tokenizer.batch_decode(next_token[0], skip_special_tokens=True)[0],
            )
