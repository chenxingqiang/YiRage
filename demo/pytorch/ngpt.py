...
rms_norm1 = torch.nn.RMSNorm(
    n_local_heads * head_dim + 2 * n_local_kv_heads * head_dim, device=_DEVICE, dtype=torch.float16
)
rms_norm2 = torch.nn.RMSNorm(intermediate_size * 2, device=_DEVICE, dtype=torch.float16)
silu = torch.nn.SiLU()
...
if __name__ == "__main__":
    ...
    alpha = torch.rand(
        batch_size * num_tokens, intermediate_size * 2, dtype=torch.float16, device=_DEVICE
    )
    for _ in range(16):
