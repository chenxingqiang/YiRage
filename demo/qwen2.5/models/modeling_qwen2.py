...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, input_layernorm, hidden_state, stream: torch.cuda.Stream = None):
        if hidden_state.shape[-2] == 1 and self.enable_yirage:
            ...
            output = self.kernel1(
                inputs=(hidden_state, input_layernorm.weight, self.fused_weight), stream=stream
            )[0]
            gate_output, up_output = torch.chunk(output, 2, -1)
            output = self.kernel2(
                inputs=(gate_output, up_output, self.down_proj.weight), stream=stream
            )[0]
        else:
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if q_len == 1 and self.enable_yirage:
            # use yirage kernels for decoding
            xqkv = self.kernel(
                inputs=(hidden_states, input_layernorm.weight, self.fused_weight), stream=stream
            )[0]
        ...
        if q_len > 1:
            ...
            fl_attn_output = decode_wrapper.run(
                query_states[0], (self.key_cache[self.layer_idx], self.value_cache[self.layer_idx])
            )
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config: Qwen2Config):
        ...
        workspace_buffer = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device=_DEVICE)
        kv_page_indices = torch.arange(1).int().to("cuda")
        kv_page_indptr = torch.tensor([0, 1], dtype=torch.int32, device=_DEVICE)
