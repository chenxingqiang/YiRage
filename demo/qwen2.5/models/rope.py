...
def rotary_kernel(
    ...
    tl.store(out_q_ptr + out_q_offset, q_out_first, mask=s_mask[:, None] & rk_half_mask[None, :])
    tl.store(
    ...
    if pid_h < num_kv_heads:
        ...
        tl.store(
            out_k_ptr + out_k_offset, k_out_first, mask=s_mask[:, None] & rk_half_mask[None, :]
        )
        tl.store(
...
def apply_rotary_pos_emb_triton(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    BLOCK_K = (
        32 if head_dim <= 32 else (64 if head_dim <= 64 else (128 if head_dim <= 128 else 256))
    )
