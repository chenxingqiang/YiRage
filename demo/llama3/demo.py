...
def parse_arguments():
    ...
    parser.add_argument("--output-dir", type=str, default=None, help="Output files directory")
    ...
    parser.add_argument(
        "--max-num-batched-tokens", type=int, default=8, help="Max number of tokens in a batch"
    )
    parser.add_argument(
        "--max-num-batched-requests", default=4, type=int, help="Max number of requests in a batch"
    )
    ...
    parser.add_argument(
        "--ngram-size", default=3, type=int, help="Ngram size for lookahead spec decode"
    )
    parser.add_argument(
        "--max-seq-length", default=512, type=int, help="Max sequence length for spec decode"
    )
    parser.add_argument("--spec-length", default=3, type=int, help="Spec length for spec decode")
    parser.add_argument(
    ...
    with torch.device("cuda"):
        if args.model_path is not None:
            ...
            model = Llama3ForCausalLM.from_pretrained(
                model_name, world_size, max_num_pages=args.max_num_pages, page_size=args.page_size
            ).to("cuda")
...
def prepare_input_tensors(model, tokenizer, messages, args, use_yirage=True):
    ...
    tokens = torch.full((num_requests, args.max_seq_length), 0, dtype=torch.long, device=_DEVICE)
    for r in range(num_requests):
        for i in range(model_inputs.input_ids.shape[-1]):
            tokens[r, i] = model_inputs.input_ids[0, i]
    prompt_lengths = torch.full(
        (num_requests,), model_inputs.input_ids.shape[-1], dtype=torch.int, device=_DEVICE
    )
    ...
    step = torch.full((num_requests,), 0, dtype=torch.int32, device=_DEVICE)
    num_new_tokens = torch.full((num_requests,), 1, dtype=torch.int32, device=_DEVICE)
...
def create_persistent_kernel(args, world_size, rank, input_data, config, eos_token_id_for_yirage):
    ...
    if args.profiling:
        ...
        profiler_tensor = torch.zeros(
            3000 * block_dim_for_profiler, dtype=torch.uint64, device=_DEVICE
        ).contiguous()
    else:
...
def create_intermediate_tensors(ypk, config, spec_decode_config, world_size, args):
    ...
    tensors["argmax_out"] = ypk.attach_input(
        torch_tensor=ypk.meta_tensors["output_tokens"], name="output_token"
    )
...
def add_transformer_layer(
    ypk, layer_idx, layer, x, inputs, tensors, config, world_size, spec_decode_config, model, args
):
    import yirage as yr
    ...
    w_q = ypk.attach_input(
        torch_tensor=layer.self_attn.q_proj.weight, name=f"layer_{layer_idx}_q_proj"
    )
    w_k = ypk.attach_input(
        torch_tensor=layer.self_attn.k_proj.weight, name=f"layer_{layer_idx}_k_proj"
    )
    w_v = ypk.attach_input(
        torch_tensor=layer.self_attn.v_proj.weight, name=f"layer_{layer_idx}_v_proj"
    )
    ...
    k_cache = ypk.attach_input(
        torch_tensor=model.model.kv_cache[0][layer_idx], name=f"layer_{layer_idx}_k_cache"
    )
    v_cache = ypk.attach_input(
        torch_tensor=model.model.kv_cache[1][layer_idx], name=f"layer_{layer_idx}_v_cache"
    )
    ...
    w_o = ypk.attach_input(
        torch_tensor=layer.self_attn.o_proj.weight, name=f"layer_{layer_idx}_o_proj"
    )
    ...
    w_gate_proj = ypk.attach_input(
        torch_tensor=layer.mlp.gate_proj.weight, name=f"layer_{layer_idx}_gate_proj"
    )
    w_up_proj = ypk.attach_input(
        torch_tensor=layer.mlp.up_proj.weight, name=f"layer_{layer_idx}_up_proj"
    )
    ...
    w_down = ypk.attach_input(
        torch_tensor=layer.mlp.down_proj.weight, name=f"layer_{layer_idx}_down_proj"
    )
...
def add_final_layers(ypk, x, model, config, tensors, spec_decode_config, args):
    ...
    w_proj = ypk.attach_input(torch_tensor=config["lm_head_weight"], name="lm_head")
    ...
    if spec_decode_config and spec_decode_config.method == "promptlookup":
        argmax_partial_grid_dim = (
            max_factor_leq_n(
                config["padded_vocab_size"], 96 // (spec_decode_config.spec_length + 1)
            ),
...
def build_yirage_graph(model, args, world_size, rank, input_data, eos_token_id_for_yirage):
    ...
    tensors = create_intermediate_tensors(ypk, config, spec_decode_config, world_size, args)
