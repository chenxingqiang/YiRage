...
# same
def naive_attention(q, key_cache, value_cache, kv_len, layer_idx, is_causal=True, enable_gqa=True):
...
class Llama3Attention(nn.Module):
    def __init__(
        ...
        self.q_proj = nn.Linear(
            config.hidden_size,
            (config.num_attention_heads // world_size) * self.head_dim,
            bias=config.attention_bias,
        )
        self.k_proj = nn.Linear(
            config.hidden_size,
            (config.num_key_value_heads // world_size) * self.head_dim,
            bias=config.attention_bias,
        )
    ...
    def forward(
        ...
        if q_len > 1:
            attn_output = naive_attention(
                q, self.key_cache, self.value_cache, q_len, self.layer_idx, True, True
            )
        else:
            kv_seq_len = step.item() + 1
            attn_output = naive_attention(
                q, self.key_cache, self.value_cache, kv_seq_len, self.layer_idx, False, True
            )
...
class Llama3DecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = Llama3Attention(
            config=config, kv_cache=kv_cache, layer_idx=layer_idx, world_size=world_size
        )
...
class Llama3Model(Llama3PreTrainedModel):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                Llama3DecoderLayer(config, self.kv_cache, layer_idx, world_size)
        ...
        self.kv_last_page_len = torch.tensor([0], dtype=torch.int32, device=_DEVICE)
