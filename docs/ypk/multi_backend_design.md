...
### 4.3 setup.py / Cython macros (aligned with CMake)

The C/C++ preprocessor definitions passed into Cython extensions must match what `CMakeLists.txt` applies to `yirage_runtime` (`target_compile_definitions`). The **single source of truth** for that mapping is:

- **`python/yirage/cmake_macros.py`** — `parse_config_cmake(config_file)` reads every `set(NAME ON|OFF)` line from the repo-root `config.cmake`; `macros_from_config(config_file)` returns a `define_macros` list (tuples `(name, value)`) mirroring the CMake logic, including fingerprint macros (`YIRAGE_FINGERPRINT_USE_*`), compatibility names (`YIRAGE_BACKEND_USE_CUDA`, `__ASCEND__`, etc.), `YIRAGE_ENABLE_PARALLEL_SEARCH` when appropriate, and `YIRAGE_USE_FORMAL_VERIFIER` / `YIRAGE_MLIR_ENABLED` when those options are ON.

At the repository root, **`setup.py`** loads `python/yirage/cmake_macros.py` with **`importlib.util.spec_from_file_location`** (not `import yirage.cmake_macros`), so Cython `define_macros` are computed **before** `yirage.core` exists. The installed package still exposes `yirage.cmake_macros` for callers that import `yirage` after a full build.

`get_backend_macros` delegates to `macros_from_config` / `parse_config_cmake` on that loaded module and prints enabled `USE_*` flags.

Standalone Cython builds using **`python/cython_setup.py`** use the same importlib load and `macros_from_config(repo/config.cmake)` for each `Extension`. Regression tests live in **`tests/python/test_cmake_macros.py`**.
