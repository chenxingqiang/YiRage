...
- **Loop R73（YiRage main，2D GEMM 激活 + rms batch=2，PR 待合并）**：闸门：图级/正确性（闭合 2D gemm GELU/ReLU/SiLU + rms_norm_linear batch=2 推理契约）。``gemm_gelu_batch2``；``gemm_relu_batch2``；``gemm_silu_batch2``；``rms_norm_linear_batch2``；builders + parity。验证：**302 passed**。
