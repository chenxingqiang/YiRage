...
- **Loop R10（YiRage main，scaled self_attention，PR 待合并）**：闸门：图级/正确性（标准 ``1/sqrt(d)`` dot-product attention）。``self_attention(..., head_dim=)`` / ``scale=`` + ``kn_mul_scalar``；``self_attention_scaled`` builder + parity。验证：``make test-cpu-value-verify`` **121 passed**。
