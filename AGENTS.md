...
| **vLLM paged multistep** | PagedAttention 多步 decode hidden + greedy token parity + native tier（G7 链 C paged） | `run_vllm_paged_multistep_bench` | `benchmark/serving_vllm_paged_multistep_bench.py`；`test_runtime_fusion_s37_*` / `s40_*` | **partial（S37/S40）** |
