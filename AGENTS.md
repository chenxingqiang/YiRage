...
- **Loop R44（YiRage main，3D standalone softmax/RMSNorm，PR 待合并）**：闸门：图级/正确性（闭合 3D 归一化原语）。扩展 ``softmax`` 3D（``_kn_softmax_batched_last_dim``）；``kn_softmax_3d``；``kn_softmax_3d_batch1``；``kn_rms_norm_3d``；``kn_rms_norm_3d_batch1``。验证：**186 passed**。（``self_attention_online_3d`` 延后：TB matmul 暂不支持 3D×2D rank mismatch。）
