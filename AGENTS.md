...
- **Loop R118（YiRage main，depthwise_bias batch/silu FAST_PATH + unit smoke，PR 待合并）**：闸门：图级/正确性（depthwise_bias batch/silu/relu_batch1 FAST_PATH + fused API smoke）。``conv2d_depthwise_bias_batch1_fast``；``conv2d_depthwise_bias_batch2_fast``；``conv2d_depthwise_bias_silu_fast``；``conv2d_depthwise_bias_relu_batch1_fast``；``graph.py`` doc + ``test_graph_conv2d_fused_api``。验证：**482 passed**。
