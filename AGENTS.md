...
- **Loop R84（YiRage main，rms_matmul + plain conv2d CV 命名，PR 待合并）**：闸门：图级/正确性（``kn_unfused_rms_matmul`` 迁入 CUSTOMIZED + 无 bias ``conv2d`` batch1/batch2 推理契约）。``kn_unfused_rms_matmul``；``conv2d``；``conv2d_batch1``；``conv2d_batch2``；registry + parity。验证：**346 passed**。
