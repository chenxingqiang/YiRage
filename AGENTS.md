...
- **Loop R70（YiRage main，online + 3D MLP + 2D layernorm batch=2，PR 待合并）**：闸门：图级/正确性（闭合 online attention + gated MLP SiLU 3D + 2D layernorm/GELU batch=2）。``self_attention_online_batch2``；``gated_mlp_3d_batch2``；``gemm_layernorm_batch2``；``gemm_layernorm_gelu_batch2``；builders + parity。验证：**290 passed**。
