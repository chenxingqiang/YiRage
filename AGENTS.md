...
- **Loop R110（YiRage main，conv2d_bias 激活 FAST_PATH 余量，PR 待合并）**：闸门：图级/正确性（``conv2d_bias_gelu/silu_fast`` + ``conv2d_bias_relu_batch1/batch2_fast``）。FAST_PATH + parity。验证：**450 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：R111 验证；**R112 实现**候选（``conv2d_bias_groups`` base/batch FAST_PATH + unit smoke）。
