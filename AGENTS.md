...
- **Loop R14（YiRage main，kn_conv2d groups，PR 待合并）**：闸门：执行层（depthwise/grouped CV）。``KN_CONV2D_OP`` JSON + API 增 ``groups``（``in_channels == weight_C_in * groups``）；CPU 解释器 ``F.conv2d(groups=)``；``conv2d_groups`` builder + parity。验证：**125 passed**。
