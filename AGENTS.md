...
- **Loop R15（YiRage main，conv2d bias+groups 复合，PR 待合并）**：闸门：图级/正确性（闭合 R13/R14 CV 栈）。``conv2d_depthwise_bias``（``groups=in_channels``）；``conv2d_bias_groups`` / ``conv2d_depthwise_bias`` builders + parity。FlashAttention online rescale 全矩阵 softmax 留 R16（tile output_map 约束）。验证：**127 passed**。
