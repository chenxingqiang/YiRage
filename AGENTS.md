...
- **Loop R63（YiRage main，LLM batch=2 命名闭合，PR 待合并）**：闸门：图级/正确性（闭合 3D GELU/gated MLP/rms ReLU + scaled attention batch=2）。``gemm_gelu_3d_batch2``；``gated_mlp_batched_batch2``；``rms_norm_linear_3d_relu_batch2``；``self_attention_scaled_3d_batch2``；builders + parity。验证：**262 passed**。
