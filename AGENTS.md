...
- **Loop R58（YiRage main，2D layernorm act batch1 + CV SiLU/groups batch=2，PR 待合并）**：闸门：图级/正确性（闭合 2D LayerNorm ReLU/SiLU batch1 + CV N=2 SiLU/grouped 推理契约）。``gemm_layernorm_relu_batch1``；``gemm_layernorm_silu_batch1``；``conv2d_bias_silu_batch2``；``conv2d_bias_groups_batch2``；builders + parity。验证：**242 passed**。
