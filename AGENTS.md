...
- **Loop R79（YiRage main，CV depthwise act + separable batch=1，PR 待合并）**：闸门：图级/正确性（闭合 depthwise ReLU/GELU/SiLU + separable batch=1 推理契约）。``conv2d_depthwise_bias_relu_batch1``；``conv2d_depthwise_bias_gelu_batch1``；``conv2d_depthwise_bias_silu_batch1``；``conv2d_separable_batch1``；builders + parity。验证：**326 passed**。
