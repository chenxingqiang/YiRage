...
- **Loop R54（YiRage main，2D softmax/bias/rms GELU batch=1，PR 待合并）**：闸门：图级/正确性（2D 注意力分数 + bias 激活 + rms GELU batch=1 命名闭合）。``gemm_softmax_batch1``；``gemm_softmax_scaled_batch1``；``rms_norm_linear_gelu_batch1``；``gemm_bias_relu_batch1``；builders + parity。验证：**226 passed**。
