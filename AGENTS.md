...
- **Loop R77（YiRage main，CV batch=1 + KN matmul batch=2，PR 待合并）**：闸门：图级/正确性（闭合 CV conv bias/groups batch=1 命名 + 2D KN matmul batch=2）。``conv2d_bias_batch1``；``conv2d_groups_batch1``；``conv2d_bias_relu_batch1``；``kn_matmul_batch2``；builders + parity。验证：**318 passed**。
