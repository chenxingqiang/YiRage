...
- **Loop R83（YiRage main，KN matmul 3d×2d + transpose CUSTOMIZED，PR 待合并）**：闸门：图级/正确性（``[B,M,K]@[K,N]`` broadcast matmul 三档 + ``kn_transpose_01`` 迁入 CUSTOMIZED）。``kn_matmul_3d_2d``；``kn_matmul_3d_2d_batch1``；``kn_matmul_3d_2d_batch2``；``kn_transpose_01``；registry + parity。验证：**342 passed**。
