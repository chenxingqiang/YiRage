...
- **Loop R72（YiRage main，2D gemm_bias batch=2 命名闭合，PR 待合并）**：闸门：图级/正确性（闭合 2D GEMM+bias 及 ReLU/GELU/SiLU 激活 batch=2 推理契约）。``gemm_bias_batch2``；``gemm_bias_relu_batch2``；``gemm_bias_gelu_batch2``；``gemm_bias_silu_batch2``；builders + parity。验证：**298 passed**。
