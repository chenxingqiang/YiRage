...
- **Loop R32（YiRage main，rms_norm_linear ReLU/SiLU 融合，PR 待合并）**：闸门：图级/正确性（闭合 rms_norm_linear 激活矩阵）。``rms_norm_linear_relu``；``rms_norm_linear_silu``；builders + parity。验证：**152 passed**。
