...
- **Loop R75（YiRage main，2D KN softmax/layer_norm/rms batch 命名，PR 待合并）**：闸门：图级/正确性（闭合 2D KN softmax batch=1 + layer_norm batch=1/2 + rms_norm batch=1）。``kn_softmax_batch1``；``kn_layer_norm_batch1``；``kn_layer_norm_batch2``；``kn_rms_norm_batch1``；builders + parity。验证：**310 passed**。
