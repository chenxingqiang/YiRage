...
- **Loop R78（YiRage main，CV act/groups/depthwise batch=1，PR 待合并）**：闸门：图级/正确性（闭合 CV GELU/SiLU/groups/depthwise batch=1 推理契约）。``conv2d_bias_gelu_batch1``；``conv2d_bias_silu_batch1``；``conv2d_bias_groups_batch1``；``conv2d_depthwise_bias_batch1``；builders + parity。验证：**322 passed**。
