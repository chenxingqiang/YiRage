...
- **Loop R60（YiRage main，CV depthwise act + separable batch=2，PR 待合并）**：闸门：图级/正确性（闭合 depthwise GELU/SiLU + separable CV N=2 推理契约）。``conv2d_depthwise_bias_gelu_batch2``；``conv2d_depthwise_bias_silu_batch2``；``conv2d_separable_batch2``；``conv2d_separable_bias_batch2``；builders + parity。验证：**250 passed**。
