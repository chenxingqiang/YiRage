...
- **Loop R56（YiRage main，2D bias/rms SiLU + CV batch=2，PR 待合并）**：闸门：图级/正确性（闭合 2D bias/rms SiLU batch1 + CV N=2 推理契约）。``gemm_bias_silu_batch1``；``rms_norm_linear_silu_batch1``；``conv2d_bias_batch2``；``kn_conv2d_batch2_op``；builders + parity。验证：**234 passed**。
