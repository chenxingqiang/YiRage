...
- **Loop R59（YiRage main，CV groups/depthwise batch=2，PR 待合并）**：闸门：图级/正确性（闭合 grouped conv + depthwise CV N=2 推理契约）。``conv2d_groups_batch2``；``kn_conv2d_groups_batch2_op``；``conv2d_depthwise_bias_batch2``；``conv2d_depthwise_bias_relu_batch2``；builders + parity。验证：**246 passed**。
