...
- **Loop R88（YiRage main，grouped conv2d 无 bias 激活，PR 待合并）**：闸门：图级/正确性（groups=2 无 bias conv2d + ReLU/GELU/SiLU 及 ``conv2d_groups_relu_batch1``）。``conv2d_groups_relu``；``conv2d_groups_gelu``；``conv2d_groups_silu``；``conv2d_groups_relu_batch1``；builders + parity。验证：**362 passed**。
