...
| **Qwen decode bench archive** | Decode parity + latency nightly JSON | `run_serving_qwen_decode_bench_archive` | `scripts/serving_decode_bench_archive.py`；`test_runtime_fusion_s31_*` | **partial（S31）** |
| **Engine G1 regression** | vLLM + SGLang torch 全链 parity（G7 C/D） | `run_engine_g1_regression` | `benchmark/serving_engine_g1_regression.py`；`test_runtime_fusion_s32_*` | **partial（S32）** |
| **HF multistep generation** | 多步 greedy decode token parity（G7 链 B 延伸） | `run_qwen_multistep_generation_bench` | `benchmark/serving_qwen_multistep_generation_bench.py`；`test_runtime_fusion_s33_*` | **partial（S33）** |
| **Combined nightly archive** | decode + G1 + multistep + engine multistep + paged multistep 联合 JSON | `run_serving_combined_nightly_archive` | `scripts/serving_combined_nightly_archive.py`；`test_runtime_fusion_s34_*` / `s38_*` | **partial（S34/S38）** |
| **Engine native multistep** | vLLM/SGLang 多步 MLP parity（G7 链 C/D multistep） | `run_engine_native_multistep_bench` | `benchmark/serving_engine_native_multistep_bench.py`；`test_runtime_fusion_s35_*` | **partial（S35）** |
| **Unified serving dashboard** | combined archive → JSON + markdown + HTML + validate CLI | `build_serving_dashboard_from_combined_archive` | `scripts/render_serving_dashboard.py`；`scripts/validate_serving_dashboard.py`；`test_runtime_fusion_s41_*` | **partial（S36/S39/S41）** |
| **Combined nightly bundle validate** | archive + dashboard meta 联合 validate（G7 归档闭合） | `validate_serving_combined_nightly_bundle` | `scripts/validate_serving_combined_nightly_bundle.py`；`test_runtime_fusion_s42_*` / `s44_*` | **partial（S42/S44）** |
| **vLLM paged multistep** | PagedAttention 多步 decode hidden + greedy token parity + native tier（G7 链 C paged） | `run_vllm_paged_multistep_bench` | `benchmark/serving_vllm_paged_multistep_bench.py`；`test_runtime_fusion_s37_*` / `s40_*` / `s43_*` | **partial（S37/S40/S43）** |
...
- **Serving Loop S31（2026-07-31，decode bench nightly archive）**：闸门：感知/工具层。`decode_bench_archive.py` + archive/validate scripts + nightly workflow；RF **s31**。验证：pytest S31（contract）+ `make test-serving-decode-bench-nightly-profile`（yirage.core）。**价值：中价值｜G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 B 归档）** — HF prefill → decode parity + latency JSON；`parity_ok` 为合并闸门。拒绝项：vLLM G1 全链回归 → S32。下一轮：**S32** — vLLM/SGLang G1 回归或 multi-step generation 链。
- **Serving Loop S32（2026-07-31，Engine G1 regression — vLLM/SGLang 链 C/D）**：闸门：图级/正确性层。`engine_g1_regression.py` 统一 torch 路径 vLLM hybrid + SGLang ForwardBatch hybrid；RF **s32**。验证：pytest S32 + `benchmark/serving_engine_g1_regression.py --quick`。**价值：中高价值｜G1+G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 C+D torch）** — 引擎 Attention/KV/Radix meta → RF MLP → parity；真 vllm/sglang 仅报告 availability。拒绝项：multi-step generation 全链 → S33。下一轮：**S33** — HF multi-step decode generation 链 B 延伸或 native vllm/sglang tier。
- **Serving Loop S33（2026-07-31，HF multi-step greedy generation）**：闸门：图级/正确性层。`qwen_multistep_generation_bench.py` — native ``generate`` vs ``greedy_decode_with_rf_mlp`` token parity；RF **s33**。验证：pytest S33（contract + slow torch/yirage_cpu）。**价值：中高价值｜G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 B 多步）** — prefill → N 步 RF MLP decode → token ids 对齐；闭合 S27 单步与 S32 引擎路径之间的 generation gap。拒绝项：native vllm/sglang multistep → S34。下一轮：**S34** — vLLM/SGLang 真引擎 multistep 或 decode+G1 联合 nightly。
- **Serving Loop S34（2026-07-31，Combined nightly archive — decode+G1+multistep）**：闸门：感知/工具层。`combined_nightly_archive.py` 联合 S31/S32/S33 为单一 validated JSON；RF **s34**；CI nightly `make test-serving-combined-nightly-profile`。验证：pytest S34（contract + slow full archive）。**价值：中高价值｜G5+G6+G7**。**功能闭合：G7 全链归档闭合（链 A/B/C/D）** — 单 artifact 含 decode parity、引擎 G1 regression、multistep token parity；非 registry 堆砌。拒绝项：native vllm/sglang multistep tier → S35。下一轮：**S35** — 真引擎 multistep 或 unified serving dashboard。
- **Serving Loop S35（2026-07-31，Engine native multistep — 链 C/D multistep）**：闸门：图级/正确性层。`engine_native_multistep_bench.py` N 步 vLLM/SGLang hybrid + 可选 native tier；G1 regression 增 native 执行；combined archive 增 ``engine_multistep``；RF **s35**。验证：pytest S35 + bench CLI。**价值：中高价值｜G1+G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 C/D multistep）** — 引擎 meta 逐步演化 → RF MLP 逐步 parity；native tier 报告 ``native_parity_ok``。拒绝项：full LLM native generation → S36。下一轮：**S36** — unified serving dashboard 或 vLLM paged multistep generation。
- **Serving Loop S36（2026-08-03，Unified serving dashboard）**：闸门：感知/工具层。`serving_dashboard.py` 从 combined archive 生成 JSON + markdown 摘要；``make test-serving-combined-nightly-profile`` 产出 ``serving-dashboard.*``；RF **s36**。验证：pytest S36 + render CLI。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合** — 单视图 merge gate + 各链 metrics；非零散 validate 脚本堆砌。拒绝项：vLLM paged full generation → S37。下一轮：**S37** — vLLM paged multistep token parity 或 dashboard HTML。
- **Serving Loop S37（2026-08-03，vLLM paged multistep token parity）**：闸门：图级/正确性层。`vllm_paged_multistep_bench.py` N 步 evolving ``VllmPagedKvBatchSpec`` → hybrid vs engine hidden parity + greedy token match；RF **s37**。验证：pytest S37 + bench CLI。**价值：中高价值｜G1+G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 C paged multistep）** — paged KV meta 逐步演化 → RF MLP 逐步 parity + token ids 对齐。拒绝项：combined archive/dashboard 接入 → S38。下一轮：**S38** — wire paged_multistep 进 combined archive/dashboard 或 dashboard HTML。
- **Serving Loop S38（2026-08-03，paged multistep in combined archive + dashboard）**：闸门：感知/工具层。combined archive 增 ``paged_multistep`` 子节 + dashboard 行；metadata ``paged_multistep_*``；RF **s38**。验证：pytest S38 + S34/S36 synthetic。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合（链 C paged）** — nightly 单 artifact 含 paged multistep token parity。拒绝项：dashboard HTML → S39。下一轮：**S39** — dashboard HTML 或 full native vLLM paged generation。
- **Serving Loop S39（2026-08-03，Serving dashboard HTML）**：闸门：感知/工具层。`render_serving_dashboard_html` + ``--html-output``；nightly profile 产出 ``serving-dashboard.html``；RF **s39**。验证：pytest S39 + render CLI。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合（HTML 视图）** — CI artifact 可浏览器 triage；非 registry 堆砌。拒绝项：full native vLLM paged generation → S40。下一轮：**S40** — native vLLM paged generation e2e 或 dashboard 上传 artifact 契约。
- **Serving Loop S40（2026-08-03，Native vLLM paged multistep tier）**：闸门：图级/正确性层。`vllm_paged_multistep_bench` 增 optional native vLLM MLP + evolving paged meta；``native_parity_ok`` 报告；combined metadata ``paged_multistep_native_parity_ok``；RF **s40**。验证：pytest S40（torch gate + skipif vllm）。**价值：中高价值｜G1+G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 C paged native）** — 真 vLLM 单 layer MLP + paged KV meta 多步 parity；cert 仍以 torch 路径为闸门。拒绝项：full-layer native paged generation → S41。下一轮：**S41** — full-layer native vLLM paged decode 或 dashboard artifact validate CLI。
- **Serving Loop S41（2026-08-03，Dashboard artifact validate CLI）**：闸门：感知/工具层。`validate_serving_dashboard_artifact_bundle` + ``scripts/validate_serving_dashboard.py``；nightly profile 产出 ``serving-dashboard.meta.json``；RF **s41**。验证：pytest S41 + validate CLI。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合（artifact 契约）** — JSON/HTML/markdown 三件套可 validate；非 registry 堆砌。拒绝项：full-layer native paged decode → S42。下一轮：**S42** — full-layer native vLLM paged decode 或 combined archive 含 dashboard meta 联合 validate。
- **Serving Loop S42（2026-08-03，Combined nightly bundle validate）**：闸门：感知/工具层。`validate_serving_combined_nightly_bundle` + ``scripts/validate_serving_combined_nightly_bundle.py``；nightly profile 产出 ``serving-nightly-bundle.meta.json``；RF **s42**。验证：pytest S42 + bundle validate CLI。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合（bundle 契约）** — archive meta + dashboard meta 交叉校验 + merge_gate 对齐；非 registry 堆砌。拒绝项：full-layer native vLLM paged decode → S43。下一轮：**S43** — full-layer native vLLM paged decode 或 nightly bundle CI contract tests。
- **Serving Loop S43（2026-08-03，Full-layer native vLLM paged multistep）**：闸门：图级/正确性层。`vllm_paged_multistep_bench` 增 ``native_full_layer_parity_ok`` — 全 layer vLLM MLP hook + evolving paged meta；combined metadata ``paged_multistep_native_full_layer_parity_ok``；RF **s43**。验证：pytest S43（torch gate + skipif vllm native full layer）。**价值：中高价值｜G1+G3+G5+G6+G7**。**功能闭合：G7 段落闭合（链 C paged native full-layer）** — 真 vLLM 每层 MLP + 多步 paged KV parity；cert 仍以 torch 路径为闸门。拒绝项：真 vLLM 全 decoder（含 Attention）多步 generation → S44。下一轮：**S44** — vLLM 全 decoder native generation 或 nightly bundle CI contract 扩展。
- **Serving Loop S44（2026-08-03，Nightly bundle CI contract）**：闸门：感知/工具层。`serving_nightly_bundle_ci_contract()` + bundle metadata ``paged_multistep_native_full_layer_parity_ok`` 交叉校验；dashboard paged 行增 ``native_full_layer_parity_ok``；RF **s44**。验证：pytest S44 + integration contract。**价值：中价值｜G5+G6+G7**。**功能闭合：G7 归档消费闭合（CI 契约）** — nightly artifact 清单 + validate 路径可审计；非 registry 堆砌。拒绝项：真 vLLM 全 decoder native generation → S45。下一轮：**S45** — vLLM 全 decoder native multistep generation。
- **Serving Loop S45（2026-08-03，Native decoder paged multistep）**：闸门：图级/正确性（闭合 S44 backlog 真 vLLM 全 decoder）。`vllm_e2e` 增 ``_vllm_chain_native_decoder_layers*``；``vllm_paged_multistep_bench`` 增 ``native_decoder_*`` tier（native attn+MLP vs RF-on-MLP + greedy token match）；archive/bundle/dashboard metadata 增 ``paged_multistep_native_decoder_*``；RF **s45**。验证：pytest S45 + S44 bundle 契约。**价值：高价值｜G7**。**功能闭合：G7 chain C paged native decoder multistep** — 自包含 native tier，非 torch cert gate。下一轮：**S46** — multi-layer native decoder generation e2e vs torch 或 MPK serving backlog。
