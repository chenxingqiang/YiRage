...
- **Loop R62（YiRage main，3D GEMM/attention batch=2 命名闭合，PR 待合并）**：闸门：图级/正确性（闭合 3D softmax/scaled + rms GELU + self_attention batch=2 契约）。``gemm_softmax_3d_batch2``；``gemm_softmax_scaled_3d_batch2``；``rms_norm_linear_3d_gelu_batch2``；``self_attention_3d_batch2``；builders + parity。验证：**258 passed**。
