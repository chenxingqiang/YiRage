...
- **Loop R115（YiRage main，separable_bias batch/silu FAST_PATH + unit smoke，PR 待合并）**：闸门：图级/正确性（separable_bias silu/batch/relu_batch1 FAST_PATH + fused API smoke）。``conv2d_separable_bias_silu_fast``；``conv2d_separable_bias_batch1_fast``；``conv2d_separable_bias_batch2_fast``；``conv2d_separable_bias_relu_batch1_fast``；``graph.py`` doc + ``test_graph_conv2d_fused_api``。验证：**470 passed**。
