...
- **Loop R117（YiRage main，separable silu batch2 + depthwise_bias FAST_PATH，PR 待合并）**：闸门：图级/正确性（``conv2d_separable_bias_silu_batch2_fast`` + depthwise_bias base ReLU/GELU FAST_PATH）。builders + parity。验证：**478 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：**R118 实现**候选（depthwise_bias batch/silu FAST_PATH + unit smoke）。
