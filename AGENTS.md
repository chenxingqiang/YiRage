...
- **Loop R90（YiRage main，groups silu batch2 + rms_matmul batched + depthwise，PR 待合并）**：闸门：图级/正确性（``conv2d_groups_silu_batch2``、plain ``conv2d_depthwise``、unfused rms_matmul batched 三档 ``[B,M,K]@[K,N]``）。``conv2d_groups_silu_batch2``；``conv2d_depthwise``；``kn_unfused_rms_matmul_batched``；``kn_unfused_rms_matmul_batched_batch2``；builders + parity。验证：**370 passed**。
