...
- **Loop R45（YiRage main，3D batch=1 快路径 builders，PR 待合并）**：闸门：图级/正确性（对称 R44 batch1 模式）。``kn_layer_norm_3d_batch1``；``gemm_layernorm_3d_batch1``；``gemm_softmax_3d_batch1``；``gemm_bias_3d_batch1``。验证：**190 passed**。（``self_attention_online_3d`` 仍延后：TB forloop/input_map 与 3D Q/K/V 契约未对齐。）
