...
- **Loop R86（YiRage main，plain conv2d 激活 batch 命名，PR 待合并）**：闸门：图级/正确性（闭合无 bias conv2d 激活 batch1/batch2 推理契约）。``conv2d_relu_batch2``；``conv2d_gelu_batch1``；``conv2d_gelu_batch2``；``conv2d_silu_batch1``；builders + parity。验证：**354 passed**。
