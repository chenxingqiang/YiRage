...
- **Loop R71（YiRage main，2D layernorm act + gated MLP batch=2，PR 待合并）**：闸门：图级/正确性（闭合 2D layernorm ReLU/SiLU + 2D gated MLP SiLU/GELU batch=2）。``gemm_layernorm_relu_batch2``；``gemm_layernorm_silu_batch2``；``gated_mlp_batch2``；``gated_mlp_gelu_batch2``；builders + parity。验证：**294 passed**。
