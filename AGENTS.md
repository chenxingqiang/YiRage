...
- **Loop R93（YiRage main，plain depthwise 激活 batch2 + separable relu，PR 待合并）**：闸门：图级/正确性（闭合 depthwise ReLU/GELU/SiLU batch2 + 无 bias separable ReLU 推理契约）。``conv2d_depthwise_relu_batch2``；``conv2d_depthwise_gelu_batch2``；``conv2d_depthwise_silu_batch2``；``conv2d_separable_relu``；builders + parity。验证：**382 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：**R94 实现**（候选：`Graph.conv2d_relu` 等无 bias fused API，或 `cpu_rms_matmul` 识别 `[B,M,K]` batched unfused 图，或 `cpu_search_explore_not_supported()` 非空 gap）。
