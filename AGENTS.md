...
- **Loop R94（YiRage main，无 bias fused conv2d Graph API，PR 待合并）**：闸门：图级/正确性（对称 ``conv2d_bias_*``，新增 ``conv2d_relu/gelu/silu``、``conv2d_depthwise*``、``conv2d_separable_*`` fused API；builders 迁移 + separable gelu/silu batch1）。``conv2d_separable_gelu``；``conv2d_separable_silu``；``conv2d_separable_relu_batch1``；``conv2d_separable_gelu_batch1``；``graph.py`` + unit smoke。验证：**386 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：R95 验证或 ``cpu_rms_matmul`` batched unfused 快路径等实现候选。
