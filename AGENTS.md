...
- **Loop R104（YiRage main，depthwise 激活 FAST_PATH batch 闭合，PR 待合并）**：闸门：图级/正确性（plain depthwise ReLU/GELU/SiLU batch1/batch2 FAST_PATH 余量）。``conv2d_depthwise_gelu_batch1_fast``；``conv2d_depthwise_silu_batch1_fast``；``conv2d_depthwise_relu_batch2_fast``；``conv2d_depthwise_gelu_batch2_fast``；FAST_PATH + parity。验证：**426 passed**。
- **Loop R105（YiRage main，depthwise silu batch2 + separable FAST_PATH，PR 待合并）**：闸门：图级/正确性（``conv2d_depthwise_silu_batch2_fast`` + plain separable ReLU/GELU/SiLU FAST_PATH）。builders + parity。验证：**430 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：**R106 实现**候选（``conv2d_separable`` 内层 ``conv2d_depthwise`` 迁移 + separable batch FAST_PATH）。
