...
- **Loop R53（YiRage main，2D GEMM/RMSNorm/attention batch=1，PR 待合并）**：闸门：图级/正确性（2D LLM 线性/注意力 batch=1 命名闭合）。``self_attention_batch1``；``rms_norm_linear_batch1``；``gemm_gelu_batch1``；``gemm_bias_batch1``；builders + parity。验证：**222 passed**。
