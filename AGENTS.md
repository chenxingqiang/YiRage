...
- **Loop R100（YiRage main，grouped bias fused Graph API，PR 待合并）**：闸门：图级/正确性（``conv2d_bias_groups*`` fused API + FAST_PATH 四档 + builders 迁移）。``conv2d_bias_groups_relu_fast``；``conv2d_bias_groups_gelu_fast``；``conv2d_bias_groups_silu_fast``；``conv2d_bias_groups_relu_batch1_fast``；``graph.py`` + unit smoke。验证：**410 passed**。
