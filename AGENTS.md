...
- **Loop R38（YiRage main，3D LLM FFN/QKV 契约扩展，PR 待合并）**：闸门：图级/正确性（R37 broadcast 落地）。``gated_mlp_3d_gelu``；``rms_norm_linear_3d``（``[B,S,D]`` + 2D 权重）；builders + parity。验证：**163 passed**。
