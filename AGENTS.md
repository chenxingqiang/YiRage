...
- **Loop R89（YiRage main，grouped conv2d 激活 batch 命名，PR 待合并）**：闸门：图级/正确性（闭合 groups=2 无 bias 激活 batch1/batch2 推理契约）。``conv2d_groups_relu_batch2``；``conv2d_groups_gelu_batch1``；``conv2d_groups_gelu_batch2``；``conv2d_groups_silu_batch1``；builders + parity。验证：**366 passed**。
