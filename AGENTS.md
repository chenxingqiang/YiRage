...
- **Loop R11（YiRage main，multi-head self_attention 3D，PR 待合并）**：闸门：图级/正确性（``[H,S,D]`` 多头契约）。``self_attention_multi_head``（chunk → per-head stable softmax → concat）；``_kn_customized_tb_softmax_last_dim`` 支持 ``[1,S,S]`` leading batch；``self_attention_multi_head`` builder + parity。验证：**122 passed**。
