...
- **Loop R95（YiRage main，separable 激活 batch 命名闭合，PR 待合并）**：闸门：图级/正确性（闭合无 bias separable SiLU batch1 + ReLU/GELU/SiLU batch2 推理契约）。``conv2d_separable_silu_batch1``；``conv2d_separable_relu_batch2``；``conv2d_separable_gelu_batch2``；``conv2d_separable_silu_batch2``；builders + parity。验证：**390 passed**。
