...
- **Loop R85（YiRage main，plain conv2d 激活 + relu batch1，PR 待合并）**：闸门：图级/正确性（无 bias ``conv2d`` + ReLU/GELU/SiLU 及 ``conv2d_relu_batch1`` 推理契约）。``conv2d_relu``；``conv2d_gelu``；``conv2d_silu``；``conv2d_relu_batch1``；builders + parity。验证：**350 passed**。
