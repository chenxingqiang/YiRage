...
- **Loop R98（YiRage main，grouped bias 激活 batch 命名，PR 待合并）**：闸门：图级/正确性（闭合 groups=2 bias 激活 batch1/batch2 余量）。``conv2d_bias_groups_gelu_batch1``；``conv2d_bias_groups_silu_batch1``；``conv2d_bias_groups_relu_batch2``；``conv2d_bias_groups_gelu_batch2``；builders + parity。验证：**402 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：R98–R99 验证（bias_groups 激活 batch 余量 / separable 命名）；**R100 实现**候选。
