...
- **Loop R122（YiRage main，groups 激活 batch FAST_PATH 余量，PR 待合并）**：闸门：图级/正确性（groups=2 ReLU/GELU/SiLU batch1/batch2 FAST_PATH 余量）。``conv2d_groups_relu_batch2_fast``；``conv2d_groups_gelu_batch1/batch2_fast``；``conv2d_groups_silu_batch1_fast``；FAST_PATH + parity。验证：**498 passed**。
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：R123 验证；**R124 实现**候选（groups 激活 builders 迁移 ``conv2d_groups_*`` + KN FAST_PATH）。
