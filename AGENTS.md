...
- **Loop R9（YiRage main，stable self_attention CPU，PR 待合并）**：闸门：图级/正确性（闭合 COMET ``self_attention`` naive softmax）。``KNGraph.self_attention`` 改用 ``_kn_customized_tb_softmax_last_dim``（与 ``gemm_softmax`` 同 stable 路径）；新增 ``self_attention`` value-verify builder + general ML parity。验证：``make test-cpu-cert-quick`` **60 passed**；``make test-cpu-value-verify`` **120 passed**。
