...
- **Loop R50（YiRage main，3D gated MLP + batched attention batch=1，PR 待合并）**：闸门：图级/正确性（3D FFN batch1 快路径 + per-batch K/V attention batch1）。``gated_mlp_3d_batch1``；``gated_mlp_3d_gelu_batch1``；``gated_mlp_batched_batch1``；``self_attention_batched_batch1``；builders + parity。验证：**210 passed**。
