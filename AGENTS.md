...
- **Loop R51（YiRage main，scaled batched scores + multi-head batch=1，PR 待合并）**：闸门：图级/正确性（闭合 batched 注意力分数/FFN GELU batch1 + KN 3D×2D matmul batch1）。``gemm_softmax_scaled_batched_batch1``；``gated_mlp_batched_gelu_batch1``；``self_attention_multi_head_batch1``；``kn_matmul_3d_2d_batch1_op``；builders + parity。验证：**214 passed**。
