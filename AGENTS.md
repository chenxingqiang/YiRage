...
- **Loop R91（YiRage main，plain depthwise batch + 激活，PR 待合并）**：闸门：图级/正确性（无 bias depthwise batch1/batch2 + ReLU/GELU 推理契约）。``conv2d_depthwise_batch1``；``conv2d_depthwise_batch2``；``conv2d_depthwise_relu``；``conv2d_depthwise_gelu``；builders + parity。验证：**374 passed**。
