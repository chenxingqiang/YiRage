...
- **Loop R76（YiRage main，KN matmul/conv/rms batch 命名，PR 待合并）**：闸门：图级/正确性（闭合 2D KN rms_norm batch=2 + matmul/conv2d/groups batch=1 推理契约）。``kn_rms_norm_batch2``；``kn_matmul_batch1``；``kn_conv2d_batch1``；``kn_conv2d_groups_batch1``；builders + parity。验证：**314 passed**。
