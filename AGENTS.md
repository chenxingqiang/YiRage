...
- **Loop R96（YiRage main，grouped conv2d bias 激活，PR 待合并）**：闸门：图级/正确性（闭合 groups=2 bias+ReLU/GELU/SiLU 及 relu batch1 推理契约）。``conv2d_bias_groups_relu``；``conv2d_bias_groups_gelu``；``conv2d_bias_groups_silu``；``conv2d_bias_groups_relu_batch1``；builders + parity。验证：**394 passed**。
