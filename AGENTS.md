...
- **Loop 节奏（2026-06，用户确认：混合 C）**：**2 轮验证 + 1 轮实现** 交替，避免纯 registry 命名闭合凑 passed。
  ...
  - **下一批**：**R150 验证**候选（``kn_conv2d_separable_{gelu,silu}_batch2_fast`` + ``conv2d_separable_batch2_op_fast`` + separable_bias ``kn_*`` 余量）；**R151 实现**候选（separable_bias KN 激活/graph doc + unit smoke）。
