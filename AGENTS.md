...
- **Loop R35（YiRage main，gemm_softmax_scaled 注意力分数融合，PR 待合并）**：闸门：图级/正确性（``softmax(scale * Q @ K)`` 分数段）。``gemm_softmax_scaled``；builder + parity vs ``softmax(matmul / sqrt(d))``。验证：**158 passed**。
