...
- **Loop R52（YiRage main，2D attention/FFN batch=1 推理契约，PR 待合并）**：闸门：图级/正确性（2D 单序列 batch=1 命名闭合）。``self_attention_scaled_batch1``；``self_attention_online_batch1``；``gated_mlp_batch1``；``gated_mlp_gelu_batch1``；builders + parity。验证：**218 passed**。
