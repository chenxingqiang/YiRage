...
- **Loop R57（YiRage main，2D layernorm batch1 + CV act batch=2，PR 待合并）**：闸门：图级/正确性（闭合 2D GEMM+LayerNorm batch1 + CV N=2 激活推理契约）。``gemm_layernorm_batch1``；``gemm_layernorm_gelu_batch1``；``conv2d_bias_relu_batch2``；``conv2d_bias_gelu_batch2``；builders + parity。验证：**238 passed**。
