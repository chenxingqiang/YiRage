...
- **Loop R67（YiRage main，3D gemm_bias 激活 + rms batch=2，PR 待合并）**：闸门：图级/正确性（闭合 3D gemm_bias ReLU/GELU/SiLU + rms_norm_linear batch=2）。``gemm_bias_3d_relu_batch2``；``gemm_bias_3d_gelu_batch2``；``gemm_bias_3d_silu_batch2``；``rms_norm_linear_3d_batch2``；builders + parity。验证：**278 passed**。
