...
// Ascend fingerprint runs on host (CPU) for verification
// __aicore__ is only for AI Core kernel code, not for fingerprint
...
inline void compute_matmul_fingerprint(
  ...
  // Use OpenMP parallelization for large matrices (especially on Ascend)
  // Parallelize over output elements to avoid data race on C_ptr
  int total_outputs = B * M * N;
  
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 64) if(total_outputs > 1024)
#endif
  for (int idx = 0; idx < total_outputs; ++idx) {
    int b = idx / (M * N);
    ...
    FPType sum = 0;
    for (int k = 0; k < K; ++k) {
      FPType prod = compute_mul_fingerprint(A_ptr[b * M * K + i * K + k],
                                            B_ptr[b * K * N + k * N + j]);
      sum = compute_add_fingerprint(sum, prod);
    }
    C_ptr[idx] = sum;
  }
}
...
inline void compute_rms_norm_fingerprint(FPType *input_ptr,
                                         ...
                                         int num_samples,
                                         int norm_size) {
  // Use OpenMP parallelization over samples (especially on Ascend)
  // Each sample is independent, so this is embarrassingly parallel
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 8) if(num_samples > 16)
#endif
