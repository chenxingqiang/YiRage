...
public:
  ...
  DTensor *relu(DTensor const *input);
  DTensor sigmoid(DTensor const &input);
  DTensor *sigmoid(DTensor const *input);
  DTensor log(DTensor const &input);
  DTensor *log(DTensor const *input);
  DTensor
  ...
  // ==========================================================================
  // Additional collective operations for distributed compound operations.
  // Reference: COMET paper (Negi et al.)
  
  // AllGather: Gather data from all participants
  DTensor all_gather(DTensor const &input, int num_participants, int gather_dim);
  KNOperator *create_all_gather_op(DTensor const &input, 
                                   int num_participants, 
                                   int gather_dim);
  
  // ReduceScatter: Reduce then scatter result
  DTensor reduce_scatter(DTensor const &input, 
                         int num_participants, 
                         int scatter_dim,
                         yirage::type::CollectiveReduceOp reduce_op = 
                             yirage::type::REDUCE_SUM);
  KNOperator *create_reduce_scatter_op(DTensor const &input,
                                       int num_participants,
                                       int scatter_dim,
                                       yirage::type::CollectiveReduceOp reduce_op);
  
  // Broadcast: One-to-all distribution
  DTensor broadcast(DTensor const &input, int num_participants, int source_rank = 0);
  KNOperator *create_broadcast_op(DTensor const &input,
                                  int num_participants,
                                  int source_rank);
  
  // Point-to-point: Send/Recv
  void p2p_send(DTensor const &input, int dest_rank);
  KNOperator *create_p2p_send_op(DTensor const &input, int dest_rank);
  
  DTensor p2p_recv(std::vector<int> const &dims, 
                   yirage::type::DataType dtype,
                   int source_rank);
  KNOperator *create_p2p_recv_op(std::vector<int> const &dims,
                                 yirage::type::DataType dtype,
                                 int source_rank);
  
  // split operator (1 input -> 2 outputs along dim)
  std::vector<DTensor> split(DTensor const &input, int split_size, int dim);
  int split(DTensor const *input, int split_size, int dim);
  KNOperator *create_split_op(DTensor const &input, int split_size, int dim);
  // concat operator
  DTensor concat(DTensor const &input1, DTensor const &input2, int concat_dim);
  DTensor *concat(DTensor const *input1, DTensor const *input2, int concat_dim);
  KNOperator *create_concat_op(DTensor const &input1,
                               DTensor const &input2,
                               int concat_dim);
  // chunk operator
...
public:
  ...
  // This flag indicates that YiRage will not compute fingerprint
  // for this kernel_graph and therefore bypass all kernel-level
  // memory check. This flag is mainly useful for the YiRage runtime
  ...
  std::map<yirage::type::GuidType, yirage::runtime::IODesc> io_config;
  std::unordered_map<yirage::kernel::KNOperator const *,
