...
public:
  ...
  void *dmem_fp_ptr;  // Device memory for fingerprint (on NPU)
  void *smem_fp_ptr;  // L1 buffer (shared memory equivalent, on NPU)
  size_t dmem_fp_size;
  size_t smem_fp_size;
  size_t dmem_fp_offset;
  size_t smem_fp_offset;
  bool use_npu_memory;  // True if memory is on NPU, false if fallback to CPU
