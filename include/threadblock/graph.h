...
#include "threadblock/serializer/kernel_params.h"
...
public:
  ...
  STensor *relu(STensor const *A);
  STensor sigmoid(STensor const &A);
  STensor *sigmoid(STensor const *A);
  STensor log(STensor const &A);
  STensor *log(STensor const *A);
  ...
  std::vector<STensor> split(STensor const &input, int split_size, int dim);
  std::vector<STensor *> split(STensor const *input, int split_size, int dim);
  TBOperator *create_split_op(STensor const &input, int split_size, int dim);
  // chunk operator (1 input -> N equal outputs along dim)
  std::vector<STensor> chunk(STensor const &input, int chunk_size, int dim);
  std::vector<STensor *> chunk(STensor const *input, int chunk_size, int dim);
  TBOperator *create_chunk_op(STensor const &input, int chunk_size, int dim);
