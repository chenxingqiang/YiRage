...
/**
 * @brief Huawei Ascend NPU/GPU backend
 * 
 * Supports Ascend 910, 910B, 310P and future generations
 * Uses CANN (Compute Architecture for Neural Networks) runtime
 */
class AscendBackend : public BackendInterface {
public:
  AscendBackend();
  virtual ~AscendBackend() = default;
...
private:
  bool check_ascend_availability();
  int detect_device_type();  // 910, 910B, 310P, etc.

  bool is_available_;
  int current_device_;
  int device_type_;  // 910=0, 910B=1, 310P=2, etc.
};
