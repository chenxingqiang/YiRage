...
// Configurations for the YPK runtime
// #define YPK_MAX_NUM_BATCHED_REQUESTS 16
// #define YPK_MAX_NUM_BATCHED_TOKENS 64
// #define YPK_MAX_NUM_PAGES 1024
// #define YPK_PAGE_SIZE 64
...
__global__ void init_kernel(RuntimeConfig config) {
  ...
  if (threadIdx.x == 0) {
    ...
    *config.next_request_id = 0;
    ...
    for (int i = 0; i < YPK_MAX_NUM_BATCHED_REQUESTS + 1; i++) {
      config.qo_indptr_buffer[i] = 0;
      config.paged_kv_indptr_buffer[i] = 0;
    }
    // Page manager
    *config.page_queue_head = 0;
    *config.page_queue_tail = YPK_MAX_NUM_PAGES;
...
__device__ __forceinline__ bool
  ...
  __shared__ int smem_kv_indices[YPK_MAX_NUM_PAGES];
...
#ifdef YPK_ENABLE_PROFILING
      if (true) {
#else
      if ((step + num_tokens + 1 >= config.max_seq_length) ||
          ((config.tokens[request_id * YPK_MAX_SEQ_LENGTH + step +
  ...
  int num_pages = config.paged_kv_indptr_buffer[YPK_MAX_NUM_BATCHED_REQUESTS];
  ...
  for (int i = 0; i < YPK_MAX_NUM_BATCHED_REQUESTS; i++) {
    ...
    if (request_id != -1) {
      ...
      if (num_new_tokens > 0) {
        // Prefill requests
        num_new_tokens =
            min(num_new_tokens, YPK_MAX_NUM_BATCHED_TOKENS - num_tokens);
      } else {
        // Decode requests
        num_new_tokens = min(1, YPK_MAX_NUM_BATCHED_TOKENS - num_tokens);
      }
      ...
      int num_new_pages =
          (step + num_new_tokens + YPK_PAGE_SIZE - 1) / YPK_PAGE_SIZE;
  ...
  *config.page_queue_tail = page_queue_tail;
  ...
  //        config.step[2],
  //        config.step[3],
  //        config.qo_indptr_buffer[YPK_MAX_NUM_BATCHED_REQUESTS]);
...
__device__ __forceinline__ void execute_worker(RuntimeConfig config) {
  ...
  constexpr int TASK_DESCS_BUFFER_LENGTH = std::min(
      (yirage::runtime::WORKER_RESERVED_STATIC_SHARED_MEMORY_SIZE - 56) /
...
extern "C" void init_persistent_kernel(std::vector<void *> meta_tensors,
  ...
  global_runtime_config.request_ids =
      gpu_malloc<int>(sizeof(int) * (YPK_MAX_NUM_BATCHED_REQUESTS + 1));
  global_runtime_config.next_request_id = gpu_malloc<int>(sizeof(int));
  global_runtime_config.page_queue =
      gpu_malloc<int>(YPK_MAX_NUM_PAGES * sizeof(int));
