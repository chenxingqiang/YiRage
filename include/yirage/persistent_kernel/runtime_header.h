...
#if defined(YIRAGE_GRACE_HOPPER) || defined(YIRAGE_GRACE_BLACKWELL)
...
struct TensorDesc {
  ...
  void *tma_desc_ptrs[yirage::config::MAX_TMA_DESC_PER_TENSOR];
#endif
  int data_type;
  int dim[yirage::config::MAX_TENSOR_DIMS];
  int stride[yirage::config::MAX_TENSOR_DIMS];
};
...
struct alignas(16) TaskDesc {
...
#ifdef YPK_ENABLE_TMA
    for (int i = 0; i < t.num_inputs; i++) {
      for (int k = 0; k < yirage::config::MAX_TMA_DESC_PER_TENSOR; k++) {
  ...
  void *input_tma_desc_ptrs[MAX_INPUTS_PER_TASK]
                           [yirage::config::MAX_TMA_DESC_PER_TENSOR];
...
#else
  void *worker_stream, *scheduler_stream;  // Placeholder for non-CUDA backends
#endif
};
