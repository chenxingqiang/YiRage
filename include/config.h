...
// ============================================================================
// Enable parallel search for CUDA/MACA backends
// OpenMP is used for multi-threaded search to maximize CPU utilization
// Note: Parallel search is disabled for Ascend backend to avoid race conditions
// in fingerprint verification. This is a temporary workaround until per-thread
// fingerprint buffers are implemented.
#if !defined(__ASCEND__) && !defined(YIRAGE_FINGERPRINT_USE_ASCEND)
...
// Huawei Ascend NPU - fingerprint runs on CPU (not NPU HBM)
// Use reasonable CPU memory limits for fingerprint verification
size_t const MAX_DMEM_FP_SIZE = (size_t)256 * 1024 * 1024; // 256 MB (CPU RAM)
size_t const MAX_SMEM_FP_SIZE = (size_t)1024 * 1024;       // 1 MB per thread-block
#elif defined(YIRAGE_FINGERPRINT_USE_CPU)
// CPU-only build - fingerprint runs entirely in host RAM
size_t const MAX_DMEM_FP_SIZE = (size_t)256 * 1024 * 1024; // 256 MB (host RAM)
size_t const MAX_SMEM_FP_SIZE = (size_t)1024 * 1024;       // 1 MB per thread-block
#else
