...
"""CPU certification profile helpers (Loop R61)."""
...
def test_planned_value_verify_still_254():
    assert planned_value_verify_count() == 254
...
def test_cert_profile_from_stages_includes_walkthrough_substage_elapsed():
    ...
    profile = cert_profile_from_stages(stages, planned_value_verify=254)
...
def test_cert_profile_from_stages_stage_elapsed_and_alignment():
    stages = {
        "value_verify_all": {
            "ok": True,
            "elapsed_s": 2.5,
            "pytest": {"passed": 254, "skipped": 1, "failed": 0},
        },
