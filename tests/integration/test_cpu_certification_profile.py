...
def test_parse_pytest_summary_passed_and_skipped():
    stdout = "....\n110 passed, 1 skipped in 1.26s\n"
    stats = parse_pytest_summary(stdout)
    assert stats["passed"] == 110
...
def test_planned_value_verify_still_110():
    assert planned_value_verify_count() == 110
...
def test_cert_profile_from_stages_includes_walkthrough_substage_elapsed():
    ...
    profile = cert_profile_from_stages(stages, planned_value_verify=110)
...
def test_cert_profile_from_stages_stage_elapsed_and_alignment():
    stages = {
        "value_verify_all": {
            "ok": True,
            "elapsed_s": 2.5,
            "pytest": {"passed": 110, "skipped": 1, "failed": 0},
        },
