...
@pytest.mark.cpu
def test_conv2d_bias_activation_batch2_fused_builds_graph(yirage_core):
    import yirage as yr
    ...
    w = g.new_input(dims=(4, 3, 3, 3), dtype=yr.float16)
    b = g.new_input(dims=(1, 4, 1, 1), dtype=yr.float16)
    relu_out = g.conv2d_bias_relu(x, w, b, stride=(1, 1), padding=(1, 1))
    gelu_out = g.conv2d_bias_gelu(x, w, b, stride=(1, 1), padding=(1, 1))
    silu_out = g.conv2d_bias_silu(x, w, b, stride=(1, 1), padding=(1, 1))
...
@pytest.mark.cpu
def test_conv2d_bias_groups_activation_batch2_fused_builds_graph(yirage_core):
    import yirage as yr
    ...
    relu_out = g.conv2d_bias_groups_relu(x, w, b, stride=(1, 1), padding=(1, 1))
    gelu_out = g.conv2d_bias_groups_gelu(x, w, b, stride=(1, 1), padding=(1, 1))
    silu_out = g.conv2d_bias_groups_silu(x, w, b, stride=(1, 1), padding=(1, 1))
    g.mark_output(relu_out)
    g.mark_output(gelu_out)
    g.mark_output(silu_out)
    assert g.cygraph is not None
...
@pytest.mark.cpu
def test_conv2d_separable_activation_batch1_fused_builds_graph(yirage_core):
    import yirage as yr
    ...
    dw = g.new_input(dims=(4, 1, 3, 3), dtype=yr.float16)
    pw = g.new_input(dims=(8, 4, 1, 1), dtype=yr.float16)
    relu_out = g.conv2d_separable_relu(x, dw, pw, stride=(1, 1), padding=(1, 1))
    gelu_out = g.conv2d_separable_gelu(x, dw, pw, stride=(1, 1), padding=(1, 1))
    silu_out = g.conv2d_separable_silu(x, dw, pw, stride=(1, 1), padding=(1, 1))
    g.mark_output(relu_out)
    g.mark_output(gelu_out)
    g.mark_output(silu_out)
    assert g.cygraph is not None
...
@pytest.mark.cpu
def test_conv2d_separable_bias_activation_batch2_fused_builds_graph(yirage_core):
    import yirage as yr
