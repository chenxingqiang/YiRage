...
def get_norm1():
    ...
    D = graph.matmul(X, W)
    O = graph.rms_norm(
        D, normalized_shape=(n_local_heads * head_dim + 2 * n_local_kv_heads * head_dim,)
    )
    graph.mark_output(O)
...
def get_norm2():
    ...
    return graph.superoptimize(
        backend="maca", previous_checkpoint=f"ngpt_norm2_maca_bs{batch_size}.json"
    )
...
def main():
    ...
    Kcache = torch.rand(
        num_kv_tokens, n_local_kv_heads, head_dim, dtype=torch.float16, device=device
    )
    Vcache = torch.rand(
        num_kv_tokens, n_local_kv_heads, head_dim, dtype=torch.float16, device=device
    )
    alpha = torch.rand(
        batch_size * num_tokens, intermediate_size * 2, dtype=torch.float16, device=device
    )
