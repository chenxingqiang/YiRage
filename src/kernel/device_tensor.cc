...
bool DTensor::has_same_fingerprint(cpu::CTensor const &ref) const {
  ...
  // Safety check: fp_offset should be non-negative after allocation
  if (fp_offset < 0) {
    std::cerr << "[ERROR] has_same_fingerprint: fp_offset is negative (" 
              << fp_offset << "), tensor not properly allocated!" << std::endl;
    return false;
  }
  ...
  // Safety check: ensure fp_base_ptr is valid
  if (dmm == nullptr || dmm->fp_base_ptr[0] == nullptr) {
    std::cerr << "[ERROR] has_same_fingerprint: DeviceMemoryManager not initialized!" << std::endl;
    return false;
  }
