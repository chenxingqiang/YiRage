...
// Note: For ASCEND backend, fingerprint is defined in src/kernel/ascend/rms_norm_kernel.ascend
#if defined(YIRAGE_FINGERPRINT_USE_CPU)
bool KNRMSNormOp::fingerprint(void) {
  // Safety checks for fp_offset
  if (input_tensors[0].fp_offset < 0 || output_tensors[0].fp_offset < 0) {
    std::cerr << "[ERROR] KNRMSNormOp::fingerprint: Invalid fp_offset "
              << "(input=" << input_tensors[0].fp_offset 
              << ", output=" << output_tensors[0].fp_offset << ")" << std::endl;
    return false;
  }
  ...
  if (dmm->div_p_lookup_table == nullptr || dmm->div_q_lookup_table == nullptr ||
      dmm->sqrt_p_lookup_table == nullptr || dmm->sqrt_q_lookup_table == nullptr) {
  ...
  for (int device_id = 0; device_id < dmm->num_gpus; ++device_id) {
    if (dmm->fp_base_ptr[device_id] == nullptr) {
      std::cerr << "[ERROR] KNRMSNormOp::fingerprint: fp_base_ptr[" << device_id << "] is null" << std::endl;
      return false;
    }
