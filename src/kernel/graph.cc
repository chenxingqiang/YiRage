...
#include "kernel/task_register.h"
  ...
  for (json const &jop : j) {
    ...
    switch (op_type) {
      ...
      case type::KNOperatorType::KN_MUL_SCALAR_OP: {
        ...
        jop.at("input_tensors")[0].at("guid").get_to(guid);
        jop.at("output_tensors")[0].at("guid").get_to(guidO);
        if (jop.contains("scalar")) {
          jop.at("scalar").get_to(scalar);
        }
        DTensor const &output =
            g.mul_scalar(get_tensor_from_guid(guid), scalar);
        guid_mapping[output.guid] = guidO;
        break;
      }
      ...
      case type::KNOperatorType::KN_RMS_NORM_OP: {
        ...
        if (jop.contains("normalized_shape")) {
          jop.at("normalized_shape").get_to(normalized_shape);
        } else {
          // Backward compat: derive from the input tensor's last dimension.
          DTensor const &input = get_tensor_from_guid(guid);
          normalized_shape.push_back(input.dim[input.num_dims - 1]);
        }
        DTensor const &output =
      ...
      case type::KNOperatorType::KN_CONCAT_2_OP: {
        ...
        jop.at("input_tensors")[1].at("guid").get_to(guidB);
        jop.at("output_tensors")[0].at("guid").get_to(guidO);
        if (jop.contains("concat_dim")) {
          jop.at("concat_dim").get_to(concat_dim);
        } else {
          concat_dim =
              (int)op_type - (int)type::KNOperatorType::KN_CONCAT_0_OP;
        }
        DTensor const &output = g.concat(get_tensor_from_guid(guidA),
                                         get_tensor_from_guid(guidB),
                                         concat_dim);
        guid_mapping[output.guid] = guidO;
        break;
      }
      ...
      case type::KNOperatorType::KN_SPLIT_2_OP: {
        size_t guid;
        int split_size, split_dim;
        jop.at("input_tensors")[0].at("guid").get_to(guid);
        jop.at("split_size").get_to(split_size);
        if (jop.contains("split_dim")) {
          jop.at("split_dim").get_to(split_dim);
        } else {
          split_dim = (int)op_type - (int)type::KNOperatorType::KN_SPLIT_0_OP;
        }
        std::vector<DTensor> outputs =
            g.split(get_tensor_from_guid(guid), split_size, split_dim);
      ...
      case type::KNOperatorType::KN_CHUNK_2_OP: {
        ...
        jop.at("input_tensors")[0].at("guid").get_to(guid);
        jop.at("chunk_size").get_to(chunk_size);
        if (jop.contains("chunk_dim")) {
          jop.at("chunk_dim").get_to(chunk_dim);
        } else {
          chunk_dim = (int)op_type - (int)type::KNOperatorType::KN_CHUNK_0_OP;
        }
        std::vector<DTensor> outputs =
            g.chunk(get_tensor_from_guid(guid), chunk_size, chunk_dim);
        for (size_t i = 0; i < outputs.size(); ++i) {
          size_t guidO;
          jop.at("output_tensors")[i].at("guid").get_to(guidO);
          guid_mapping[outputs[i].guid] = guidO;
