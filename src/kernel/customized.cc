...
#include "threadblock/operator.h"
  ...
  for (auto const &op : _graph.operators) {
    ...
    switch (op->op_type) {
      ...
      case yirage::type::TB_CLAMP_OP: {
        assert(my_inputs.size() == 1);
        if (auto const *clamp_op =
                dynamic_cast<yirage::threadblock::TBClampUnaryOp const *>(op)) {
          bgraph.elementunary_clamp(
              my_inputs[0], clamp_op->min_val, clamp_op->max_val);
        } else {
          assert(!type::CLAMP_MIN_MAX.empty());
          bgraph.elementunary_clamp(my_inputs[0],
                                    type::CLAMP_MIN_MAX.at("min_val"),
                                    type::CLAMP_MIN_MAX.at("max_val"));
        }
        break;
      }
      ...
      case yirage::type::TB_SIGMOID_OP:
      case yirage::type::TB_LOG_OP:
      ...
      case yirage::type::TB_SPLIT_0_OP:
      case yirage::type::TB_SPLIT_1_OP:
      case yirage::type::TB_SPLIT_2_OP: {
        assert(my_inputs.size() == 1);
        int split_dim = op->op_type - yirage::type::TB_SPLIT_FIRST_OP_ID;
        auto const *split_op =
            dynamic_cast<yirage::threadblock::TBSplitOp const *>(op);
        assert(split_op != nullptr);
        bgraph.split(my_inputs[0], split_op->split_size, split_dim);
        break;
      }
      case yirage::type::TB_CHUNK_0_OP:
      case yirage::type::TB_CHUNK_1_OP:
      case yirage::type::TB_CHUNK_2_OP: {
        assert(my_inputs.size() == 1);
        int chunk_dim = op->op_type - yirage::type::TB_CHUNK_0_OP;
        auto const *chunk_op =
            dynamic_cast<yirage::threadblock::TBChunkOp const *>(op);
        assert(chunk_op != nullptr);
        bgraph.chunk(my_inputs[0], chunk_op->chunk_size, chunk_dim);
        break;
      }
  ...
  auto compute_operator = [&](TBOperator const *op,
    ...
    switch (op->op_type) {
      ...
      case type::TB_SQUARE_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_square_fingerprint(input_buffers[0][i]);
        }
        break;
      }
      case type::TB_SQRT_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_sqrt_fingerprint(
              input_buffers[0][i], dmm->sqrt_p_lookup_table, dmm->sqrt_q_lookup_table);
        }
        break;
      }
      case type::TB_EXP_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_exp_fingerprint(
              input_buffers[0][i], dmm->exp_lookup_table);
        }
        break;
      }
      case type::TB_SILU_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_silu_fingerprint(
              input_buffers[0][i], dmm->exp_lookup_table);
        }
        break;
      }
      case type::TB_GELU_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_gelu_fingerprint(
              input_buffers[0][i], dmm->exp_lookup_table);
        }
        break;
      }
      case type::TB_RELU_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_relu_fingerprint(input_buffers[0][i]);
        }
        break;
      }
      case type::TB_CLAMP_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_clamp_fingerprint(input_buffers[0][i]);
        }
        break;
      }
      ...
      case type::TB_ADD_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_add_fingerprint(
              input_buffers[0][i], input_buffers[1][i]);
        }
        break;
      }
      case type::TB_MUL_OP: {
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_mul_fingerprint(
              input_buffers[0][i], input_buffers[1][i]);
        }
        break;
      }
      ...
      case type::TB_REDUCTION_2_OP: {
        ...
        int num_dims = op->input_tensors[0].num_dims;
        int reduce_size = op->input_tensors[0].dim[reduce_dim];
        int outer_size = 1, inner_size = 1;
        for (int d = 0; d < reduce_dim; ++d) {
          outer_size *= op->input_tensors[0].dim[d];
        }
        for (int d = reduce_dim + 1; d < num_dims; ++d) {
          inner_size *= op->input_tensors[0].dim[d];
        }
        for (int outer = 0; outer < outer_size; ++outer) {
          for (int inner = 0; inner < inner_size; ++inner) {
            FPType sum = 0;
            for (int r = 0; r < reduce_size; ++r) {
              int in_idx = outer * reduce_size * inner_size + r * inner_size + inner;
              sum = utils::compute_add_fingerprint(sum, input_buffers[0][in_idx]);
            }
            int out_idx = outer * inner_size + inner;
            output_buffers[0][out_idx] = sum;
          }
        }
        break;
      }
      ...
      case type::TB_RMS_NORM_OP: {
        int num_elements = op->output_tensors[0].num_elements();
        int norm_size = op->input_tensors[0].dim[op->input_tensors[0].num_dims - 1];
        int num_samples = num_elements / norm_size;
        utils::compute_rms_norm_fingerprint(
            input_buffers[0], output_buffers[0],
            dmm->div_p_lookup_table, dmm->div_q_lookup_table,
            dmm->sqrt_p_lookup_table, dmm->sqrt_q_lookup_table,
            num_samples, norm_size);
        break;
      }
      // MUL_SCALAR operation
      case type::TB_MUL_SCALAR_OP: {
        threadblock::TBElementUnaryOp const *unary_op =
            dynamic_cast<threadblock::TBElementUnaryOp const *>(op);
        FPType scalar_fp = static_cast<FPType>(unary_op->scalar * 1000) % config::FP_PQ;
        for (size_t i = 0; i < op->output_tensors[0].num_elements(); ++i) {
          output_buffers[0][i] = utils::compute_mul_fingerprint(
              input_buffers[0][i], scalar_fp);
        }
        break;
      }
      default: {
