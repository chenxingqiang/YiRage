 ...
 * 
 * MACA Matrix Multiplication Kernel
 * 
 * Matrix multiplication implementation for MetaX MACA backend.
 * Optimized for MACA's 64-thread warp architecture.
 */
...
/**
 * @brief Compute matrix multiplication fingerprint on MACA
 * 
 * This kernel computes C = A * B fingerprint for verification.
 * Note: Block size should be multiple of 64 (MACA warpSize).
 */
__global__ void compute_matmul_fingerprint_maca(yirage::type::FPType *A_ptr,
...
bool KNMatmulOp::fingerprint(void) {
  ...
  int const num_threads_per_blk = 1024;  // 16 warps of 64 threads
  int num_blocks =
  ...
  for (int gpu_id = 0; gpu_id < kgraph->gpu_dim.x; gpu_id++) {
    ...
    compute_matmul_fingerprint_maca<<<num_blocks, num_threads_per_blk>>>(
