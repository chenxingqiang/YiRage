...
#include "threadblock/serializer/concat_serializer.h"
#include "threadblock/serializer/element_binary_serializer.h"
#include "threadblock/serializer/element_unary_serializer.h"
#include "threadblock/serializer/forloop_accum_serializer.h"
#include "threadblock/serializer/input_loader_serializer.h"
#include "threadblock/serializer/matmul_serializer.h"
#include "threadblock/serializer/output_saver_serializer.h"
#include "threadblock/serializer/reduction_serializer.h"
#include "threadblock/serializer/rms_norm_serializer.h"
...
size_t Graph::calculate_shared_memory_usage(TBOperator *new_op) {
  ...
  // Calculate shared memory usage based on operator types
  // Note: This heuristic-based calculation provides a quick estimate.
  // For precise SMEM usage, the transpiler generates exact requirements
  // during code generation. The heuristic is used for search space pruning.
  for (auto const &op : operators) {
    switch (op->op_type) {
      ...
      // Concat / split (layout; materialize outputs in smem)
...
NewKernelParams Graph::get_new_kernel_params(bool fingerprint) const {
  ...
  for (size_t i = 0; i < operators.size(); i++) {
    ...
    switch (operators[i]->op_type) {
      ...
      case yirage::type::TB_FORLOOP_ACCUM_RED_LD_RMS_OP:
      case yirage::type::TB_FORLOOP_ACCUM_REDTOX_LD_SUM_OP: {
        // Forloop accumulation reduces along the dimension where input != accum
        // For multi-dimensional reduction, the reduction degree is computed
        // as the ratio of input to accumulator elements
        assert(operators[i]->input_tensors.size() == 1);
  ...
  for (json const &op : j["operators"]) {
    ...
    switch (op_type) {
      ...
      case type::TBOperatorType::TB_REDUCTION_2_MAX_OP: {
        int dim = op_type - type::TBOperatorType::TB_REDUCTION_0_MAX_OP;
        std::vector<STensor> outputs = graph.reduction_max(
            get_tensor_from_guid(
                op.at("input_tensors")[0].at("guid").get<int>()),
            dim);
      ...
      case type::TBOperatorType::TB_SPLIT_2_OP: {
        int dim = op_type - type::TBOperatorType::TB_SPLIT_0_OP;
        int split_size = op.at("split_size").get<int>();
        std::vector<STensor> outputs = graph.split(
            get_tensor_from_guid(op.at("input_tensors")[0].at("guid").get<int>()),
            split_size,
            dim);
        guid_mapping[outputs[0].guid] =
            op.at("output_tensors")[0].at("guid").get<int>();
      ...
      case type::TBOperatorType::TB_CHUNK_2_OP: {
        ...
        std::vector<STensor> outputs = graph.chunk(
            get_tensor_from_guid(op.at("input_tensors")[0].at("guid").get<int>()),
            chunk_size,
            dim);
        for (size_t i = 0; i < outputs.size(); ++i) {
          guid_mapping[outputs[i].guid] =
              op.at("output_tensors")[i].at("guid").get<int>();
        }
        break;
      }
