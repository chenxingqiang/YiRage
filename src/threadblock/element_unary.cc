...
STensor Graph::sigmoid(STensor const &input) {
  return elementunary(input, yirage::type::TB_SIGMOID_OP);
}
...
STensor Graph::log(STensor const &input) {
  return elementunary(input, yirage::type::TB_LOG_OP);
}
...
TBElementUnaryOp::operator json() const {
  ...
  if (op_type == yirage::type::TB_MUL_SCALAR_OP) {
    j["scalar"] = scalar;
  }
  return j;
}

TBClampUnaryOp::operator json() const {
...
bool tb_operator_clamp_bounds(TBOperator const *op,
                              float *min_val,
                              float *max_val) {
  if (op == nullptr || op->op_type != yirage::type::TB_CLAMP_OP) {
    return false;
  }
  if (auto const *cop = dynamic_cast<TBClampUnaryOp const *>(op)) {
    *min_val = cop->min_val;
    *max_val = cop->max_val;
    return true;
  }
  if (!type::CLAMP_MIN_MAX.empty()) {
    *min_val = type::CLAMP_MIN_MAX.at("min_val");
    *max_val = type::CLAMP_MIN_MAX.at("max_val");
    return true;
  }
  return false;
}

bool tb_operator_mul_scalar_value(TBOperator const *op, float *scalar) {
  if (op == nullptr || op->op_type != yirage::type::TB_MUL_SCALAR_OP) {
    return false;
  }
  *scalar = static_cast<TBElementUnaryOp const *>(op)->scalar;
  return true;
}

std::string tb_operator_json_dump(TBOperator const *op) {
  if (op == nullptr) {
    return "{}";
  }
  if (op->op_type == yirage::type::TB_CLAMP_OP) {
    auto const *cop = static_cast<TBClampUnaryOp const *>(op);
    return json(cop->operator json()).dump();
  }
  return json(op->operator json()).dump();
}
