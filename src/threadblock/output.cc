...
TBOperator *Graph::create_output_op(STensor const &stensor,
                                    ...
                                    int forloop_dim,
                                    yirage::type::TBEpilogueType epilogue) {
  // Validate that output tensor has been accumulated before saving
  // This ensures proper forloop reduction semantics are maintained.
  // Note: For distributed kernels, partial results may be output before
  // full accumulation - handle via allow_partial_output flag if needed.
