...
#include "transpiler/sched_tb_graph.h"
...
CustomOPTranspileResult
  ...
  for (TBSchedNode const &node :
    ...
    if (node.type == tb_sched_node_t::OPERATOR &&
      ...
      // TMA (Tensor Memory Access) for Hopper architecture (SM90+)
      // TMA enables hardware-accelerated async bulk memory operations
      ...
      if (use_tma && use_chunked_copy) {
        ...
        // Uses cp.async.bulk for efficient memory transfers
        string tma_descriptor = fmt("tma_descriptor_$", stensor.guid);
        code.e("// Using TMA for efficient bulk memory transfer (Hopper+)");
        code.e("// TMA descriptor: $", tma_descriptor);
      }
      
      if (use_swizzled) {
        // Swizzled shared memory layout
        int swizzle_bits = 3;  // 8-byte swizzle (common for fp16)
        code.e("// Swizzled layout: $-bit swizzle for bank conflict avoidance",
               1 << swizzle_bits);
      }
