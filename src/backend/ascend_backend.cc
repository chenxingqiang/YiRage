...
#ifdef __ASCEND__
  // Check if Ascend backend is compiled - actual availability will be
  // determined by torch_npu at runtime. Don't call ACL directly here
  // to avoid conflicts with torch_npu's ACL initialization.
  return true;
#else
  return false;
#endif
...
int AscendBackend::detect_device_type() {
  // Default to 910B (most common current device)
  // Don't call ACL functions to avoid conflicts with torch_npu
  return 1;
}
