...
KNConcatOpArgs::KNConcatOpArgs(int concat_dim) : concat_dim(concat_dim) {}

KNSplitOpArgs::KNSplitOpArgs(int split_dim, int split_size)
    : split_dim(split_dim), split_size(split_size) {}

KNChunkOpArgs::KNChunkOpArgs(int chunk_dim, int num_chunks)
    : chunk_dim(chunk_dim), num_chunks(num_chunks) {}
...
TBSplitOpArgs::TBSplitOpArgs(int split_dim, int split_size)
    : split_dim(split_dim), split_size(split_size) {}

TBChunkOpArgs::TBChunkOpArgs(int chunk_dim, int num_chunks)
    : chunk_dim(chunk_dim), num_chunks(num_chunks) {}
...
KNConcatOpArgs::operator json() const {
  return json{{"concat_dim", concat_dim}};
}

KNSplitOpArgs::operator json() const {
  return json{{"split_dim", split_dim}, {"split_size", split_size}};
}

KNChunkOpArgs::operator json() const {
  return json{{"chunk_dim", chunk_dim}, {"num_chunks", num_chunks}};
}
...
TBSplitOpArgs::operator json() const {
  return json{{"split_dim", split_dim}, {"split_size", split_size}};
}

TBChunkOpArgs::operator json() const {
  return json{{"chunk_dim", chunk_dim}, {"num_chunks", num_chunks}};
}
