...
threadblock::Graph *SymbolicTBGraph::to_threadblock_graph(
  ...
  for (size_t i = 0; i < this->operators.size(); ++i) {
    ...
    for (threadblock::STensor const &out : op->output_tensors) {
      tensors_val.push_back(out);
    }
  }
  return graph;
}
...
bool SymbolicTBGraph::add_operator(type::TBOperatorType op_type,
  ...
  switch (op_type) {
    ...
    case type::TBOperatorType::TB_SPLIT_2_OP: {
      int split_dim = (int)op_type - (int)type::TBOperatorType::TB_SPLIT_0_OP;
      assert(input_indices.size() == 1);
      SymbolicSTensor A = this->tensors[input_indices[0]];
      if ((int)A.dims.size() <= split_dim) {
        return false;
      }
      DimVarAssignments empty;
      int axis = A.dims[split_dim].dim_expr->get_value(empty);
      if (axis < 2) {
        return false;
      }
      int split_size = axis / 2;
      std::shared_ptr<TensorDimExpr const> first_dim_expr =
          dim_expr_make_div(A.dims[split_dim].dim_expr,
                            dim_expr_make_const(2));
      std::shared_ptr<TensorDimExpr const> second_dim_expr =
          dim_expr_make_div(A.dims[split_dim].dim_expr,
                            dim_expr_make_const(2));
      this->operators.push_back(
          SymbolicTBOp(op_type,
                       std::make_shared<TBSplitOpArgs>(split_dim, split_size)));
      {
        std::vector<SymbolicTensorDim> first_dims = A.dims;
        first_dims[split_dim] = SymbolicTensorDim(first_dim_expr);
        SymbolicSTensor first_out(first_dims, A.after_accum);
        this->tensors.push_back(first_out);
        std::vector<SymbolicTensorDim> second_dims = A.dims;
        second_dims[split_dim] = SymbolicTensorDim(second_dim_expr);
        SymbolicSTensor second_out(second_dims, A.after_accum);
        this->tensors.push_back(second_out);
      }
      this->input_indices.push_back(input_indices);
      this->output_indices.push_back(
          {(int)this->tensors.size() - 2, (int)this->tensors.size() - 1});
      break;
    }
    case type::TBOperatorType::TB_CHUNK_0_OP:
    case type::TBOperatorType::TB_CHUNK_1_OP:
    case type::TBOperatorType::TB_CHUNK_2_OP: {
      int chunk_dim = (int)op_type - (int)type::TBOperatorType::TB_CHUNK_0_OP;
      assert(input_indices.size() == 1);
      SymbolicSTensor A = this->tensors[input_indices[0]];
      if ((int)A.dims.size() <= chunk_dim) {
        return false;
      }
      DimVarAssignments empty;
      int axis = A.dims[chunk_dim].dim_expr->get_value(empty);
      if (axis < 2 || axis % 2 != 0) {
        return false;
      }
      int num_chunks = 2;
      std::shared_ptr<TensorDimExpr const> part_dim_expr =
          dim_expr_make_div(A.dims[chunk_dim].dim_expr,
                            dim_expr_make_const(num_chunks));
      this->operators.push_back(
          SymbolicTBOp(op_type,
                       std::make_shared<TBChunkOpArgs>(chunk_dim, num_chunks)));
      std::vector<int> out_indices;
      for (int part = 0; part < num_chunks; ++part) {
        std::vector<SymbolicTensorDim> part_dims = A.dims;
        part_dims[chunk_dim] = SymbolicTensorDim(part_dim_expr);
        out_indices.push_back((int)this->tensors.size());
        this->tensors.push_back(SymbolicSTensor(part_dims, A.after_accum));
      }
      this->input_indices.push_back(input_indices);
    ...
    case type::TBOperatorType::TB_REDUCTION_2_MAX_OP: {
      int reduction_dim =
          (int)op_type - (int)type::TBOperatorType::TB_REDUCTION_0_MAX_OP;
      assert(input_indices.size() == 1);
      SymbolicSTensor A = this->tensors[input_indices[0]];
      if ((int)A.dims.size() <= reduction_dim) {
        return false;
          ...
          op_type, std::make_shared<TBReductionOpArgs>(reduction_dim, -1)));
      {
        ...
        dim_templates[reduction_dim] =
            SymbolicTensorDim(std::make_shared<TensorDimConst>(1));
        SymbolicSTensor max_out(dim_templates, A.after_accum);
        this->tensors.push_back(max_out);
        SymbolicSTensor diff_out(dim_templates, A.after_accum);
        this->tensors.push_back(diff_out);
      }
      this->input_indices.push_back(input_indices);
      this->output_indices.push_back(
          {(int)this->tensors.size() - 2, (int)this->tensors.size() - 1});
...
bool SymbolicKNGraph::add_operator(type::KNOperatorType op_type,
  ...
  switch (op_type) {
    ...
    case type::KNOperatorType::KN_CONCAT_2_OP: {
      int concat_dim = (int)op_type - (int)type::KNOperatorType::KN_CONCAT_0_OP;
      assert(input_indices.size() == 2);
      SymbolicDTensor A = this->tensors[input_indices[0]];
      SymbolicDTensor B = this->tensors[input_indices[1]];
      if (A.dims.size() != B.dims.size()) {
      ...
      this->operators.push_back(
          SymbolicKNOp(op_type, std::make_shared<KNConcatOpArgs>(concat_dim)));
      {
        std::vector<SymbolicTensorDim> dim_templates = A.dims;
        dim_templates[concat_dim] =
            SymbolicTensorDim(dim_expr_make_add(A.dims[concat_dim].dim_expr,
                                                B.dims[concat_dim].dim_expr));
        SymbolicDTensor C(dim_templates);
        this->tensors.push_back(C);
      }
      this->input_indices.push_back(input_indices);
    ...
    case type::KNOperatorType::KN_SPLIT_2_OP: {
      int split_dim = (int)op_type - (int)type::KNOperatorType::KN_SPLIT_0_OP;
      assert(input_indices.size() == 1);
      SymbolicDTensor A = this->tensors[input_indices[0]];
      if ((int)A.dims.size() <= split_dim) {
        return false;
      ...
      this->operators.push_back(
          SymbolicKNOp(op_type,
                       std::make_shared<KNSplitOpArgs>(split_dim, split_size)));
      int first_idx = (int)this->tensors.size();
      {
        std::vector<SymbolicTensorDim> first_dims = A.dims;
        first_dims[split_dim] = SymbolicTensorDim(first_dim_expr);
        this->tensors.push_back(SymbolicDTensor(first_dims));
        std::vector<SymbolicTensorDim> second_dims = A.dims;
        second_dims[split_dim] = SymbolicTensorDim(second_dim_expr);
        this->tensors.push_back(SymbolicDTensor(second_dims));
      }
      this->input_indices.push_back(input_indices);
      this->output_indices.push_back({first_idx, first_idx + 1});
      break;
    }
    case type::KNOperatorType::KN_CHUNK_0_OP:
    case type::KNOperatorType::KN_CHUNK_1_OP:
    case type::KNOperatorType::KN_CHUNK_2_OP: {
      int chunk_dim = (int)op_type - (int)type::KNOperatorType::KN_CHUNK_0_OP;
      assert(input_indices.size() == 1);
      SymbolicDTensor A = this->tensors[input_indices[0]];
      if ((int)A.dims.size() <= chunk_dim) {
        return false;
      ...
      this->operators.push_back(SymbolicKNOp(
          op_type, std::make_shared<KNChunkOpArgs>(chunk_dim, num_chunks)));
      std::vector<int> out_indices;
      for (int part = 0; part < num_chunks; ++part) {
        std::vector<SymbolicTensorDim> part_dims = A.dims;
        part_dims[chunk_dim] = SymbolicTensorDim(part_dim_expr);
        out_indices.push_back((int)this->tensors.size());
        this->tensors.push_back(SymbolicDTensor(part_dims));
      }
      this->input_indices.push_back(input_indices);
      this->output_indices.push_back(out_indices);
      break;
