    ...
    for (threadblock::TBOperator *op : graph.operators) {
      switch (op->op_type) {
        ...
        case type::TBOperatorType::TB_SPLIT_2_OP: {
          ...
          size_t axis = op->input_tensors[0].num_dims - 1 -
                        (op->op_type - type::TBOperatorType::TB_SPLIT_0_OP);
          std::string input = tensor_exprs.at(op->input_tensors[0].guid);
          std::string size = std::to_string(split_op->split_size);
          tensor_exprs.emplace(
              op->output_tensors[0].guid,
              "(split0 " + input + " " + data_dim[axis] + " " + size + ")");
          tensor_exprs.emplace(
              op->output_tensors[1].guid,
              "(split1 " + input + " " + data_dim[axis] + " " + size + ")");
          break;
        }
        ...
        case type::TBOperatorType::TB_CHUNK_2_OP: {
          ...
          int chunk_dim =
              op->op_type - type::TBOperatorType::TB_CHUNK_0_OP;
          size_t axis = op->input_tensors[0].num_dims - 1 - chunk_dim;
          std::string input = tensor_exprs.at(op->input_tensors[0].guid);
          int part_width =
              op->input_tensors[0].dim[chunk_dim] / chunk_op->chunk_size;
          std::string size = std::to_string(part_width);
          for (size_t i = 0; i < op->output_tensors.size(); ++i) {
            std::string expr = (i == 0)
                                   ? "(split0 " + input + " " + data_dim[axis] +
                                         " " + size + ")"
                                   : "(split1 " + input + " " + data_dim[axis] +
                                         " " + size + ")";
            tensor_exprs.emplace(op->output_tensors[i].guid, expr);
          }
          break;
        }
        ...
        case type::TBOperatorType::TB_MUL_SCALAR_OP: {
          ...
          std::string input = tensor_exprs.at(op->input_tensors[0].guid);
          // The scalar is typically stored as a property; use a placeholder here
          tensor_exprs.emplace(op->output_tensors[0].guid,
                               "(mul_scalar " + input + ")");
          break;
        }
        ...
        case type::TBOperatorType::TB_REDUCTION_2_MAX_OP: {
          size_t axis = op->input_tensors[0].num_dims - 1 -
                        (op->op_type - type::TBOperatorType::TB_REDUCTION_0_MAX_OP);
          std::string a = tensor_exprs.at(op->input_tensors[0].guid);
          std::string max_expr = "(max " + a + " " + data_dim[axis] + ")";
          tensor_exprs.emplace(op->output_tensors[0].guid, max_expr);
          tensor_exprs.emplace(op->output_tensors[1].guid, max_expr);
          break;
        }
    ...
    for (kernel::KNOperator *op : graph.operators) {
      switch (op->op_type) {
        ...
        case type::KNOperatorType::KN_CONCAT_2_OP: {
          size_t axis = op->input_tensors[0].num_dims - 1 -
                        (op->op_type - type::KNOperatorType::KN_CONCAT_0_OP);
          std::string lhs = tensor_exprs.at(op->input_tensors[0].guid);
          std::string rhs = tensor_exprs.at(op->input_tensors[1].guid);
          tensor_exprs.emplace(op->output_tensors[0].guid,
                               "(concat " + lhs + " " + rhs + " " +
        ...
        case type::KNOperatorType::KN_SPLIT_2_OP: {
          ...
          size_t axis = op->input_tensors[0].num_dims - 1 -
                        (op->op_type - type::KNOperatorType::KN_SPLIT_0_OP);
          std::string input = tensor_exprs.at(op->input_tensors[0].guid);
          std::string size = std::to_string(split_op->split_size);
          tensor_exprs.emplace(
              op->output_tensors[0].guid,
        ...
        case type::KNOperatorType::KN_CHUNK_2_OP: {
          ...
          int chunk_dim =
              op->op_type - type::KNOperatorType::KN_CHUNK_0_OP;
          size_t axis =
              op->input_tensors[0].num_dims - 1 - chunk_dim;
          std::string input = tensor_exprs.at(op->input_tensors[0].guid);
          int part_width =
              op->input_tensors[0].dim[chunk_dim] / chunk_op->chunk_size;
          std::string size = std::to_string(part_width);
