...
bool MPSProfiler::load_from_file(std::string const &filename) {
  ...
  while (std::getline(file, line)) {
    ...
    // Parse CSV line: config_hash,latency_ms,memory_bytes,timestamp
    ...
    if (tokens.size() >= 3) {
      try {
        std::string config_hash = tokens[0];
        float latency_ms = std::stof(tokens[1]);
        // tokens[2] is memory_bytes but ProfileEntry uses actual_time_ms
        (void)tokens[2];  // memory_bytes not used in current ProfileEntry
        
        // Reconstruct profile entry with parsed data
        ProfileEntry entry;
        entry.actual_time_ms = latency_ms;
        entry.estimated_score = 0.0f;  // Not stored in file
        entry.problem_m = 0;
        entry.problem_n = 0;
        entry.problem_k = 0;
        
        history_.push_back(entry);
        // cache_ stores MPSKernelConfig, not ProfileEntry
        // Just populate history from file, cache is built during runtime
