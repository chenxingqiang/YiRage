...
#include "search/verification/formal_verifier.h"
...
#pragma omp task
  ...
  if (c.level == SearchLevel::LV_KERNEL) {
    ...
    for (type::KNOperatorType op_type : dim_strategy.get_knop_cand()) {
      if (count_op_of_type(type::KNOperatorType::KN_CONCAT_0_OP, *c.kn_graph) >=
              1 &&
          op_type == type::KNOperatorType::KN_CONCAT_THEN_MATMUL_OP) {
        continue;
      }
...
void KernelGraphGenerator::generate_kernel_graphs() {
  ...
  for (auto const &input_attr : computation_graph_input_attrs) {
    ...
    // Layout is derived from strides for row-major/column-major detection.
    // Both are kept for backwards compatibility and explicit layout control.
...
void KernelGraphGenerator::generate_kernel_graphs_symbolic() {
  ...
  for (auto const &input_attr : computation_graph_input_attrs) {
    auto [dim, data_type, layout, strides] = input_attr;
    // Note: layout is inferred from strides (row-major vs column-major)
    // Both are kept for API compatibility; strides take precedence
...
bool KernelGraphGenerator::check_abstract_expr(
  ...
  // Check if this expression can be rewritten to any of the final expressions
  // that were initialized in the constructor
  return subexpr_to_final_expr(expr);
}
...
#pragma omp task
  ...
  if (level == SearchLevel::LV_KERNEL) {
    ...
    for (type::KNOperatorType op_type : dim_strategy.get_knop_cand()) {
      if (count_op_of_type(type::KNOperatorType::KN_CONCAT_0_OP, *kn_graph) >=
              1 &&
          op_type == type::KNOperatorType::KN_CONCAT_THEN_MATMUL_OP) {
        continue;
      }
...
bool KernelGraphGenerator::instantiate_symbolic_graph(
  ...
  // Grid dimension and forloop range candidates for search space exploration
  // These values are tuned for common GPU architectures (sm70-sm90)
  // Custom values can be set via config.grid_dim_candidates/frange_candidates
  std::vector<int> grid_dim_cands = config.grid_dim_candidates.empty() 
      ? std::vector<int>{1, 4, 8, 16, 32, 64, 128} 
      : config.grid_dim_candidates;
  std::vector<int> frange_cands = config.frange_candidates.empty()
      ? std::vector<int>{1, 2, 4, 8, 16, 32, 64}
      : config.frange_candidates;
