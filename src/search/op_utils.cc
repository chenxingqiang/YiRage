...
bool is_unary(type::TBOperatorType op) {
  std::unordered_set<type::TBOperatorType> true_values{
      ...
      type::TBOperatorType::TB_SIGMOID_OP,
      type::TBOperatorType::TB_LOG_OP,
      ...
      type::TBOperatorType::TB_REDUCTION_0_MAX_OP,
      type::TBOperatorType::TB_REDUCTION_1_MAX_OP,
      type::TBOperatorType::TB_REDUCTION_2_MAX_OP,
      ...
      type::TBOperatorType::TB_MUL_SCALAR_OP,
      type::TBOperatorType::TB_SPLIT_0_OP,
      type::TBOperatorType::TB_SPLIT_1_OP,
      type::TBOperatorType::TB_SPLIT_2_OP,
...
bool is_unary(type::KNOperatorType op) {
  std::unordered_set<type::KNOperatorType> true_values{
      ...
      type::KNOperatorType::KN_SIGMOID_OP,
      type::KNOperatorType::KN_LOG_OP,
      type::KNOperatorType::KN_CLAMP_OP,
      type::KNOperatorType::KN_MUL_SCALAR_OP,
      type::KNOperatorType::KN_RMS_NORM_OP,
      type::KNOperatorType::KN_OUTPUT_OP,
      type::KNOperatorType::KN_SPLIT_0_OP,
      type::KNOperatorType::KN_SPLIT_1_OP,
      type::KNOperatorType::KN_SPLIT_2_OP,
...
int get_input_number(type::KNOperatorType op) {
  ...
  if (op == type::KNOperatorType::KN_CONCAT_THEN_MATMUL_OP) {
    return 4;
  }
...
KNOperator *create_op(kernel::Graph &g,
  ...
  switch (type) {
    ...
    case type::KNOperatorType::KN_MUL_SCALAR_OP:
      return g.create_elementunary_op(input, type, 0.5f);
    case type::KNOperatorType::KN_SPLIT_0_OP:
    case type::KNOperatorType::KN_SPLIT_1_OP:
    case type::KNOperatorType::KN_SPLIT_2_OP: {
      int dim = (int)type - (int)type::KNOperatorType::KN_SPLIT_0_OP;
      if (input.num_dims <= dim || input.dim[dim] < 2) {
        return nullptr;
      }
      int split_size = input.dim[dim] / 2;
      return g.create_split_op(input, split_size, dim);
    }
    case type::KNOperatorType::KN_CHUNK_0_OP:
    case type::KNOperatorType::KN_CHUNK_1_OP:
    case type::KNOperatorType::KN_CHUNK_2_OP: {
      int dim = (int)type - (int)type::KNOperatorType::KN_CHUNK_0_OP;
      if (input.num_dims <= dim || input.dim[dim] < 2 ||
          input.dim[dim] % 2 != 0) {
        return nullptr;
      }
...
KNOperator *create_op(kernel::Graph &g,
  ...
  if (type == type::KNOperatorType::KN_CONCAT_THEN_MATMUL_OP &&
      inputs.size() == 4) {
    KNOperator *concat1 =
    ...
    KNOperator *matmul = g.create_matmul_op(concat1->output_tensors[0],
...
TBOperator *create_op(threadblock::Graph &g,
  ...
  switch (type) {
    ...
    case type::TBOperatorType::TB_REDUCTION_2_MAX_OP: {
      int dim = (int)type - (int)type::TBOperatorType::TB_REDUCTION_0_MAX_OP;
      if (input.num_dims <= dim ||
          (input.num_dims > dim && input.dim[dim] == 1)) {
        return nullptr;
      }
      return g.create_reduction_max_op(input, dim);
    }
