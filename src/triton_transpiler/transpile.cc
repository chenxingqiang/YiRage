 ...
 */
#include "triton_transpiler/transpile.h"
  ...
  for (kn::KNOperator *const op : g->operators) {
    switch (op->op_type) {
      ...
      case KN_POW_OP: {
        ...
        // Compute grid dimensions based on tensor shape
        int num_dims = input0.num_dims;
        int M, N, B;  // B = batch dimensions collapsed
        
        if (num_dims == 1) {
          M = 1;
          N = CEIL_DIV(input0.dim[0], BLOCK_SIZE_X);
          B = 1;
        } else if (num_dims == 2) {
          M = CEIL_DIV(input0.dim[0], BLOCK_SIZE_Y);
          N = CEIL_DIV(input0.dim[1], BLOCK_SIZE_X);
          B = 1;
        } else {
          // ND: collapse batch dimensions
          B = 1;
          for (int d = 0; d < num_dims - 2; ++d) {
            B *= input0.dim[d];
          }
          M = CEIL_DIV(input0.dim[num_dims - 2], BLOCK_SIZE_Y);
          N = CEIL_DIV(input0.dim[num_dims - 1], BLOCK_SIZE_X);
        }

        std::string grid = (B > 1) ? fmt("($, $, $)", B, M, N) : fmt("($, $)", M, N);
        std::string new_line =
      ...
      case KN_SQUARE_OP: {
        ...
        if (num_dims == 1) {
          M = 1;
          N = CEIL_DIV(input.dim[0], BLOCK_SIZE_X);
          B = 1;
        } else if (num_dims == 2) {
          M = CEIL_DIV(input.dim[0], BLOCK_SIZE_Y);
          N = CEIL_DIV(input.dim[1], BLOCK_SIZE_X);
          B = 1;
        } else {
          B = 1;
          for (int d = 0; d < num_dims - 2; ++d) {
          ...
          M = CEIL_DIV(input.dim[num_dims - 2], BLOCK_SIZE_Y);
          N = CEIL_DIV(input.dim[num_dims - 1], BLOCK_SIZE_X);
        }
      ...
      case KN_RELU_OP:
      case KN_CLAMP_OP: {
        // Implement ReLU and clamp using Triton's tl.where and tl.minimum/maximum
        kn::DTensor &input = op->input_tensors[0];
        kn::DTensor &output = op->output_tensors[0];
        
        int num_dims = input.num_dims;
        int M = (num_dims >= 2) ? CEIL_DIV(input.dim[num_dims - 2], BLOCK_SIZE_Y) : 1;
        int N = CEIL_DIV(input.dim[num_dims - 1], BLOCK_SIZE_X);
        
        std::string grid = fmt("($, $)", M, N);
        std::string kernel_name = (op->op_type == KN_RELU_OP) ? "relu_kernel" : "clamp_kernel";
        std::string new_line = fmt("$[$]($, $, $, $)",
                                   kernel_name,
                                   grid,
                                   fmt("dtensor$", input.guid),
