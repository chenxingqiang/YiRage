...
#include "nki_transpiler/transpile.h"
#include "nki_transpiler/utils.h"
  ...
  for (tb::TBOperator *tb_op : g.operators) {
    ...
    switch (tb_op->op_type) {
      ...
      case type::TB_MATMUL_OP: {
        ...
        // Support N-dimensional tensors by treating them as batched matmul
        // For ND tensors: [..., M, K] @ [..., K, N] -> [..., M, N]
        // The batch dimensions are handled by NKI's tensor engine
        if (num_dims > 2) {
          // ND matmul: collapse batch dimensions
          code.e("# ND matmul: treating first $ dims as batch", num_dims - 2);
          // Reshape for NKI: (batch, M, K) @ (batch, K, N)
          int batch_size = 1;
          for (int d = 0; d < num_dims - 2; ++d) {
            batch_size *= input0.dim[d];
          }
          code.e("# Effective batch size: $", batch_size);
        } else if (num_dims < 2) {
          // Handle 1D case as vector operation
          code.e("# 1D tensor: treating as row vector");
        }
      ...
      case type::TB_REDUCTION_2_OP: {
        ...
        // Handle partition dim changes during reduction
        // When reducing along partition dim, output partition dim shifts
        bool partition_dim_changed = (meta0.partition_dim != meta1.partition_dim);
        if (partition_dim_changed) {
          code.e("# Note: partition dim changes from $ to $ during reduction",
                 meta0.partition_dim, meta1.partition_dim);
        }
        ...
        // Map reduction dimension to NKI axis
        // NKI uses axis=0 for partition dimension, axis=1 for free dimension
        int nki_axis = (reduc_dim == meta0.partition_dim) ? 0 : 1;
        
        if (reduc_dim == num_dims - 2) {
          code.e("$ = nl.sum($, axis=$, keepdims=True)",
                 fmt("stensor$", output.guid),
                 fmt("stensor$", input.guid),
                 nki_axis);
          ...
          // Support reduction along any dimension for ND tensors
          code.e("# Reducing along dimension $", reduc_dim);
          code.e("$ = nl.sum($, axis=$, keepdims=True)",
                 fmt("stensor$", output.guid),
                 fmt("stensor$", input.guid),
                 nki_axis);
